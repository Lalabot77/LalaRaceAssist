...
- Fixed Pit Fuel Control OFF->MAN command emission to send a valid explicit ON payload (`#+fuel$`) and aligned over-max fuel feedback wording contract (`FUEL MAX` / `AUTO FUEL … >MAX`) without changing refuel math or send strategy.
