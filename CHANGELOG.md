...
- Pit Stop Debrief now shows any finite valid completed box delta regardless of magnitude, treats `BOX ... (Δ PENDING)` as missing/invalid/not-completed source only, and uses a repair-excluded debrief box-delta basis so optional/mandatory repair time does not dominate the box-performance comparison. Repair context can still label SummaryText box sections as `BOX MAND REPAIR`, `BOX OPT REPAIR`, or `BOX REPAIRS`, while runtime `Pit.Box.LastDeltaSec` remains the separate repair-aware `target - actual` pit-box countdown/popup value.
...
- Documentation sweep: added canonical technical subsystem doc `Docs/Subsystems/Race_Awareness/League_Class_System.md` and aligned cross-references in H2H/Dash/internal docs for League Class behavior.
...
- **Monitor System setting placement** moved from Launch UI / Bindings to Settings -> Plugin Monitor. Phase 2B pit-stop monitoring now adds edge-triggered `REFUEL OFF`, `MFD FUEL LOW`, and `EXIT FUEL SHORT` warnings while preserving Phase 2A trigger evidence logs.
...
### Fixed
- Fixed Pit Stop Debrief Box Delta so repair time is excluded from both sides of the box-performance comparison; fuel/tyre/boxed-service timing is still judged, repair context remains in service/summary text, and repair-aware countdown/PitExit/Strategy timing behavior is unchanged.
