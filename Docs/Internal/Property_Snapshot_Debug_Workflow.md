...
- SimHub log hygiene: START, STOP, RESET, ignored START guards, path discovery, schema reset, and write/fallback failures remain logged. Routine auto-capture heartbeat and successful one-shot/rolling write lines are intentionally suppressed because the CSV files themselves prove successful captures.
