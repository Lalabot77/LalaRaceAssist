...
- `GlobalSettingsView.xaml` Debug Options includes `Enable Car Tracking Probe`, Probe A/B `CarIdx` inputs, `Frequency Hz`, `START`, `STOP`, and `RESET CSV` controls for a diagnostics-only CSV writer. The toggle tooltip states it enables targeted diagnostic CSV capture for player plus Probe A/B CarIdx values; the frequency tooltip documents the 1–20 Hz range.
