...
- `Global Dash Functions` -> Fuel: `Fuel Ready Confidence` sets the live-fuel readiness threshold, `Pit-in Fuel Reserve` sets the stint reserve margin, `Push/Save Profile Mode` is a two-state toggle for Pit Fuel Control PUSH/SAVE basis (OFF=`LIVE`, ON=`PROFILE`), and `Pit Fuel Push/Save Profile Guard (%)` clamps profile-assisted PUSH/SAVE burn around live stable NORM burn.
