...
- `Global Dash Functions` -> Fuel: `Fuel Ready Confidence` sets the live-fuel readiness threshold, `Pit-in Fuel Reserve` sets the stint reserve margin, `Fuel Data Plan Mode` is a two-state toggle for Pit Fuel Control DATA (OFF=`LIVE`, ON=`PLAN` for PUSH/SAVE memory/profile assumptions), and `Pit Fuel DATA PLAN Guard (%)` clamps DATA PLAN PUSH/SAVE burn around live stable NORM burn.
