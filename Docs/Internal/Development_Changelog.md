...
- `Pit.Box.LastDeltaSec` is now the plugin-owned completed-box delta seam with debrief sign convention (`actual elapsed - predicted target`; positive = slower than predicted, negative = faster) and Pit Debrief consumes that seam directly, including a post-countdown refresh so a box-exit ordering edge does not leave `BOX ... (Δ PENDING)` when the seam is valid. Implausible `abs(delta) > 10.0s` values remain suppressed.
