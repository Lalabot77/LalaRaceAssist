...
- `Pit.Box.LastDeltaSec` is now the plugin-owned completed-box delta seam with debrief sign convention (`actual elapsed - predicted target`; positive = slower than predicted, negative = faster) and Pit Debrief consumes that seam directly, including a post-countdown refresh keyed by a separate completed-delta validity flag so both exact `0.0s` on-target stops and non-zero deltas do not leave `BOX ... (Δ PENDING)` when the seam is valid. Implausible `abs(delta) > 10.0s` values remain suppressed.
- `Pit.Debrief.Service.FuelTargetLitres` now preserves the largest positive requested-add value observed from existing `Fuel.Pit.Box.WillAddLatched` / `Fuel.Pit.WillAdd` evidence so later MFD/gauge reset-to-zero ticks cannot erase the stop target. Actual added fuel remains separate and is not used as a fake target.
