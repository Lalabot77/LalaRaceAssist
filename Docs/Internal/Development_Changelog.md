...
- Dark Mode brightness actions reuse the existing brightness slider setting (`0..100`, +/-5 steps); Shift Assist toggle reuses the existing system enabled setting only; Strategy contingency actions reuse the planner `ContingencyValue` path only in Extra Litres mode and log/no-op in laps mode.
- No fuel formulas, Shift Assist learning/locks/samples, launch logic, SimHub exports/properties, or dashboard JSON were changed. Property Snapshot list reviewed: yes; no SimHub exports/properties were added, removed, renamed, behavior-recontracted, or regrouped.
