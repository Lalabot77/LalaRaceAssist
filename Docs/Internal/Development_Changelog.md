...
- Safe/normal and within-margin (+1.0 kph tolerance) entries keep the existing timing-loss performance headline (`ENTRY GOOD/NORMAL (Δ +x.xs)`). Once bad-entry speed/late evidence is captured, later pit-entry-assist reset/default refresh values do not overwrite it during collection/finalization. The Pit Entry Assist cue/braking math, box/service/strategy delta behavior, exports, logs, and dashboard layouts are unchanged. Property Snapshot list reviewed: yes; no SimHub exports/properties were added, removed, renamed, behavior-recontracted, or regrouped.
