## 2026-06-26 — v1.1 first-run default settings alignment
- Classification: **both** (user-facing first-run settings behavior plus internal persistence/default documentation; no runtime calculations, dashboard package edits, SimHub export names, or action names changed).
- Updated `LaunchPluginSettings` initializer defaults for new/missing settings: Monitor System remains ON; auto dash switching remains ON; Dark Mode defaults to Auto at 90%; Strategy Dash starts Simple (`AdvancedMode=false`); League Class and debug/runtime diagnostics remain OFF; Fuel Ready Confidence is 30%; Pit-in Fuel Reserve remains 10%; Fuel Data Plan remains LIVE/OFF; DATA PLAN guard remains 10%; pit-box optional repairs are included by default.
- Updated Dash Visibility first-run defaults so Driver/Overlay surfaces enable driving aids and suppress verbose/race-flag/radio/traffic surfaces, while Strategy Dash suppresses driving aids and enables verbose/race-flag/radio/traffic surfaces. Existing persisted settings keep saved values when present; missing fields receive the new v1.1 defaults. Property Snapshot list reviewed: yes; no SimHub exports/properties changed.
