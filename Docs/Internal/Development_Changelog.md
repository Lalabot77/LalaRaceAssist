## 2026-06-17 — Pit Entry post-line handover evidence preservation
- Classification: **both** (Pit Entry/Pit Debrief correctness for current-stop bad-entry details; no export names, BrakeCue math, pit entry line classification, 1.0 kph tolerance, Pit Debrief SummaryText logic, PitCycleLite, Fuel Model, PitExit prediction, or dashboard JSON/layout changes).
- The immediate post-line Pit Entry handover reset now disables active cue outputs without clearing latched entry-line evidence (`Pit.EntrySpeedDelta_kph`, `Pit.EntryLineLateBy_m`, `Pit.EntryLineDebrief`, `Pit.EntryLineDebriefText`, `Pit.EntryLineTimeLoss_s`, and `Pit.EntryLineDebriefSerial`) before Pit Debrief reads it later in the tick. This preserves current-stop `ENTRY BAD (+x.xkph / y.ym late)` detail even when `Pit.Box.DistanceM < 200m` or pit-box distance authority publishes `0` immediately after crossing.
- Property Snapshot list reviewed: yes; no SimHub export/property add, remove, rename, behavior-contract regroup, or new snapshot group required.
...
- Refined `Pit.EntryBrakeCueText` to the driver-facing vocabulary `OFF`/`FAULT`/`READY`/`BRAKE IN Xm`/`BRAKE NOW`/`BRAKE HARD`/`SPEED OKAY`/`BELOW LIMIT`/`TOO SLOW`, removing `BRAKE SOON` and `LATE` from the new BrakeCue text only while preserving legacy `Pit.EntryCue` and `Pit.EntryCueText`. Added `Pit.EntryBrakeCueState` (`0..8`) so dashboards can colour/animate without parsing text.
