- VersionManifestAudit exported dashboard package support landed:
  ...
  - `VersionManifestAudit.py` now opens exported dashboard packages directly without extraction, reads only the declared root `.djson`, and reports missing/invalid package, root-file, JSON, metadata, version, and mismatch failures with release-critical/non-critical separation. Runtime installed detection, embedded manifest loading, Overview UI, dashboard files/JS, and plugin exports are unchanged. Property Snapshot list reviewed: yes; no SimHub export/property names or snapshot groups changed.
- Overview dashboard package card cosmetic polish landed:
  - package cards now use short user-facing descriptions, aligned Installed/Bundled version rows, and distinct status pills for `INSTALLED`, `NOT INSTALLED`, and `UNKNOWN`; the bundled label is shortened to `Bundled` while preserving the installed/bundled distinction.
  - healthy manifest/installed-scan state now displays `Dashboard package information loaded successfully.` instead of raw embedded-manifest and scan status text. Detection, manifest loading, version comparison, dashboard files, and runtime logic are unchanged. Property Snapshot list reviewed: yes; no SimHub export/property names or snapshot groups changed.
## 2026-07-09 — Dashboard installed package root-path follow-up
- Classification: **internal bug fix with driver-facing Overview effect** (corrects local installed-dashboard detection path resolution only; no dashboard JSON/JS changes, manifest schema change, GitHub/latest logic, update comparison, recursive search, or runtime fuel/strategy/pit/H2H/CarSA/opponents/shift/launch/rejoin/monitor logic changed).
- Installed dashboard scanning now treats the loaded plugin DLL directory itself as the primary SimHub root (`<PluginDllDirectory>/DashTemplates`) and only falls back to the parent directory's `DashTemplates` for non-standard layouts. This matches the documented normal install where `LaunchPlugin.dll` sits in the SimHub root beside `DashTemplates`. Property Snapshot list reviewed: yes; no export names or snapshot groups changed.

## 2026-07-09 — Dashboard installed package detection
- Classification: **both** (driver-facing Overview package status plus internal Dashboard Management/export documentation; no dashboard JSON/JS changes, GitHub latest package check, update-available logic, auto-download/install, recursive folder search, manifest schema change, or runtime fuel/strategy/pit/H2H/CarSA/opponents/shift/launch/rejoin/monitor logic changed).
- Overview dashboard package cards now separate local installed dashboard versions from bundled plugin compatibility versions: installed versions are read from deterministic SimHub `DashTemplates/<package>/<root .djson>` `Metadata.DashboardVersion` files, while bundled versions remain sourced from the embedded manifest.
- Added cached local installed package statuses (`INSTALLED`, `NOT INSTALLED`, `UNKNOWN`) and additive per-package `Dashboards.*.InstalledVersion` / `InstalledStatusText` exports; `ExpectedVersion` and `LatestVersion` still mean the embedded bundled manifest value, with GitHub latest package checking deferred.
- The installed scan runs when Overview opens and when Check Now is pressed, not on the recurring status timer, and it does not recurse, scan arbitrary folders, edit dashboards, or download/install packages. Property Snapshot list reviewed: yes; additive `Dashboards.*` exports remain Raw Debug by default.

## 2026-07-06 — Overview dashboard package card polish
- Classification: **both** (driver-facing Overview UI polish plus internal UI docs alignment; no manifest schema, embedded-manifest load behavior, SimHub export/action names, dashboard files, installed-dashboard detection, update-check logic, warning UX, or runtime fuel/strategy/pit/H2H/CarSA/launch/rejoin/monitor behavior changed).
- Replaced the separate Included Systems, Dashboard / Overlay Versions, and Dashboard Previews Overview sections with one `DASHBOARD / OVERLAY PACKAGES` section. The section binds six package cards to existing embedded-manifest dashboard rows, shows official expected version/release-critical status/property name/preview image path, and explicitly states that the plugin does not detect installed dashboard versions. Property Snapshot list reviewed: yes; no SimHub export/property additions, removals, renames, behavior changes, or regrouping required.
...
- `VersionManifestAudit.py` now treats manifest schema errors as critical regardless of `releaseCritical`: missing/non-boolean `releaseCritical`, missing displayName/compatiblePluginFamily/folder/file/versionProperty, non-object asset schema, and malformed manifest/dashboard version strings are release-blocking audit failures. Property Snapshot list reviewed: yes; no SimHub export/property names or snapshot groups changed.

## 2026-06-30 — Dashboard manifest embedded-resource follow-up
- Classification: **both** (driver-facing Overview version-awareness reliability plus internal release-audit tightening; no installed-dashboard detection, dashboard JSON/layout/formula changes, runtime telemetry/fuel/strategy/pit/H2H/CarSA behavior, settings schema, or auto-update behavior changed).
- Runtime dashboard-version awareness now loads the manifest from the embedded `LaunchPlugin.dll` resource instead of requiring a loose `LalaRaceAssist.VersionManifest.json` beside the DLL. The root manifest remains the repo source of truth and is embedded by the project at build time.
- Manifest validation now requires the official six-asset release catalogue to be complete and valid before `Dashboards.Manifest.Valid=true`; this validates the official catalogue only and does not imply users installed all six dashboards/overlays. Overview wording now says official expected versions, notes installed dashboards may be a subset, and repeats that the plugin does not detect installed dashboard versions. Property Snapshot list reviewed: yes; existing `Dashboards.*` exports remain additive and in Raw Debug by default.

## 2026-06-27 — Dashboard expected-version exports and Overview display
- Classification: **both** (driver-facing Overview dashboard package version visibility plus SimHub export contract for dashboard self-comparison; no installed-dashboard detection, dashboard JSON/layout/formula changes, runtime telemetry/fuel/strategy/pit/H2H/CarSA behavior, settings schema, or auto-update behavior changed).
- The plugin now loads `LalaRaceAssist.VersionManifest.json` at startup as the single source of truth for dashboard/overlay expected/latest versions and safely reports missing/invalid manifest state. Added `LalaLaunch.Dashboards.Manifest.*` and per-asset `LalaLaunch.Dashboards.<Asset>.ExpectedVersion` / `LatestVersion` / `ReleaseCritical` exports for Driver Dash, Strategy Dash, Alerts Overlay, Vertical Traffic Bar, Head2Head, and Fuel Calculator.
- Overview now shows a Dashboard / Overlay Versions section with manifest status, expected version wording, and release-critical grouping. It explicitly states that installed dashboard versions are not detected and that installed version is shown inside each dashboard via dashboard-local metadata. Property Snapshot list reviewed: yes; new `Dashboards.*` exports intentionally remain in the existing Raw Debug group via default snapshot grouping.
...
- Aligned only the main/root `.djson` `DashboardVersion` metadata for the six manifest-tracked packages to v1.1.0 and added the read-only `Docs/Internal/VersionManifestAudit.py` release-prep audit. Shared copied widgets remain unversioned independently and inherit their containing package version. Property Snapshot list reviewed: no; no SimHub exports/properties were added, removed, renamed, or behavior-changed.
...
## 2026-06-26 — Pit Debrief served-repair label follow-up
- Classification: **both** (driver-facing Pit Debrief repair label correctness plus internal docs; no export names, settings, dashboard JSON/layout, runtime `Pit.Box.*` countdown/display semantics, PitExit prediction, Fuel.Live.TotalStopLoss, or Strategy timing changes).
- Changed Pit Debrief repair influence wording to derive only from served repair streams already used for repair-adjusted Box Delta: mandatory+optional served => `REPAIRS`, mandatory-only served => `MAND REPAIR`, optional-only served => `OPT REPAIR`, and no served repair => no repair influence label. Positive but static optional repair-left telemetry no longer labels a stop as `OPT REPAIR`.
- Property Snapshot list reviewed: yes; no SimHub export/property additions, removals, renames, behavior-contract changes, or regrouping required.
...
- Added a debrief-only repair remaining reader that always includes mandatory and optional repair-left telemetry so optional repairs waited out are removed from Pit Debrief Box Delta even when the runtime optional-repair countdown display setting is off. The existing setting-gated `CalculatePitBoxRepairRemainingSeconds` path continues to own runtime `Pit.Box.*` countdown/display behavior. Property Snapshot list reviewed: yes; no SimHub export/property additions, removals, renames, behavior-contract changes, or regrouping required.
## 2026-06-26 — First-lap boxed-stop stall timing follow-up
- Classification: **internal-only** (PitEngine timing coherence follow-up to the Road America phase handoff fix; no export names, dashboard JSON, pit-loss learning semantics, Pit Debrief semantics, fuel/refuel/strategy calculations, marker learning, or settings changed).
- Extracted the existing pit-stall start/stop timer edge handling into a shared helper and called it from the first-lap/pre-race pit-road suppression path before publishing live phase. This keeps `PitStopElapsedSec` live when that path legitimately publishes `CurrentPitPhase=InBox`, while pit-loss candidate learning remains suppressed until at least one completed lap. Property Snapshot list reviewed: yes; no SimHub export/property additions, removals, renames, or regrouping required.

## 2026-06-26 — Road America pit phase/Box Entry handoff fix
- Classification: **both** (driver-facing Box Entry visibility/phase correctness plus internal PitEngine lifecycle correction; no dashboard JSON, export names, pit-loss learning semantics, Pit Debrief service/box semantics, fuel/refuel/strategy calculations, or settings changed).
- Root cause: the first-lap/pre-race pit-loss suppression path returned before refreshing live `CurrentPitPhase` and Pit Entry Assist handover state. If Pit Entry Assist armed before the line, `EnteringPits` could remain latched through pit lane, leaving `RejoinCurrentPitPhase=EnteringPits`, keeping the post-line `Pit.EntryAssistActive` hold alive, and blocking Box Entry dashboard eligibility even while forward-wrapped `Pit.Box.DistanceM` was valid underneath.
- Fix: during that guarded first-lap/pre-race path, PitEngine now updates live phase and Pit Entry Assist before returning while still suppressing pit-loss learning. In-lane drive-throughs can progress `EnteringPits -> ApproachingBox -> LeavingBox -> ExitingPits` without pit-stall evidence, and boxed stops still use existing `InBox`/leaving semantics. Property Snapshot list reviewed: yes; no SimHub export/property additions, removals, renames, behavior regrouping, or dashboard formula changes were required.
...
- Pit Debrief now latches a debrief-only repair-excluded service target alongside the existing repair-aware pit-box countdown target and uses repair-adjusted driver-controlled boxed elapsed time minus that target for `Pit.Debrief.BoxDeltaSec` / `SummaryText`. The target includes modeled fuel service, tyre service, and the existing boxed-service overhead, but excludes optional and mandatory repair duration; served repair contribution is also removed from the elapsed side when repair-left evidence is available.
...
- 2026-06-26 v1.1 binding/docs release polish landed:
  ...
  - aligned PrimaryDashMode wording to `Plugin Quick Reset` / manual runtime recovery reset and cleaned internal registered-action documentation by removing stale `SecondaryDashMode` references and summarizing current action families. Property Snapshot list reviewed: no; UI wording/docs-only task with no export/property changes.
- 2026-06-26 v1.1 release identity/docs alignment landed:
  - assembly identity now reports v1.1 through `AssemblyVersion`/`AssemblyFileVersion` `1.1.0.0` with `AssemblyInformationalVersion` `1.1.0`; `LalaLaunch.Plugin.VersionNumberText` continues to resolve from the assembly name/version path.
  - README/Quick Start install wording now identifies `LaunchPlugin.dll` as the required plugin DLL and keeps `RSC.iRacingExtraProperties.dll` documented as not required for current active runtime paths or plugin-owned pit/custom command transport.
  - public v1.1 changelog notes were deduplicated and now include upgrade notes for plugin-owned pit/custom actions, no active iRacingExtraProperties fallback requirement, manual Pit Service Regulations selection, and separate dashboard package validation. No dashboard files, runtime logic, SimHub exports, action bindings, settings defaults, or packaging metadata outside assembly attributes changed. Property Snapshot list reviewed: no; release identity/docs-only task with no export/property changes.

## 2026-06-26 — Dash Control action follow-up fixes
- Classification: **both** (dashboard/button binding behavior plus internal docs alignment; no public user docs updated because the Options/Dash Control workflow is not yet a released public workflow).
- `ShiftAssistToggle` now notifies `ProfilesManagerViewModel.ShiftAssistEnabled` after changing `Settings.ShiftAssistEnabled`, keeping the open Profiles checkbox in sync while preserving the legacy `ShiftAssist_ToggleShiftAssist` alias and leaving learning/samples/targets/locks/review popups untouched.
- Strategy contingency +/- actions are now always litre controls: they switch to Extra Litres through the existing FuelCalcs contingency mode setter when needed, keep the current numeric value, apply +/-1.0 L, and clamp to `0.0..20.0` via the existing planner property path.
- Removed the prior laps-mode ignored/no-op contract from docs/log inventory. Dark Mode brightness +/- was rechecked and still uses the slider-backed `Settings.DarkModeBrightnessPct` value with `0..100` clamping; no Dark Mode mode/Lovely semantics changed. Property Snapshot list reviewed: yes; no SimHub exports/properties changed.
## 2026-06-26 — Dash Control missing action bindings
- Classification: **both** (dashboard/button binding surface is user-facing; implementation/docs are internal).
- Added SimHub/dashboard actions for `DarkModeBrightnessDecrease`, `DarkModeBrightnessIncrease`, `ShiftAssistToggle`, `StrategyContingencyAdd1L`, and `StrategyContingencyRemove1L`, and exposed them in Bindings -> General / Dash Controls for the Strategy Dash Options -> Dash Control workflow.
- Dark Mode brightness actions reuse the existing brightness slider setting (`0..100`, +/-5 steps); Shift Assist toggle reuses the existing system enabled setting only; Strategy contingency actions switch the planner to Extra Litres mode when needed, then reuse the planner `ContingencyValue` path for +/-1L adjustments.
- No fuel formulas, Shift Assist learning/locks/samples, launch logic, SimHub exports/properties, or dashboard JSON were changed. Follow-up: the Shift Assist dashboard action now notifies the Profiles checkbox ViewModel state when fired while the Profiles tab is open. Property Snapshot list reviewed: yes; no SimHub exports/properties were added, removed, renamed, behavior-recontracted, or regrouped.
...
- `Pit.Box.BrakeNow` keeps its existing legacy trigger authority/behavior; the new limiter/runway exports use native kph authority and clear to `0` before no-game/invalid-data early returns, while invalid/missing pit-box authority still fails closed to `0`/`false`. No settings, service timing, Pit Entry braking math, pit-box distance/time authority, PitExit prediction, Pit Debrief, or dashboard JSON/layout behavior changed. Property Snapshot list reviewed: yes; new `Pit.*` exports resolve to the existing Pit/PitExit group.
...
- Scope is cue-only: no new settings/exports/state IDs, no dashboard JSON/layout edits, and no changes to `Pit.Box.DistanceM`, `Pit.Box.TimeS`, pit-box authority/location, stationary detection, service timing, Pit Entry math, PitCycleLite, PitExit prediction, `Pit.Box.LastDeltaSec`, or Pit Debrief calculations. Property Snapshot list reviewed: yes; existing `Pit.Box.BrakeNow` remains in the Pit/PitExit group with no regroup required.

## 2026-06-21 — Pit Entry pit-speed unit audit and BrakeCue tolerance alignment
- Classification: **both** (driver-facing Pit Entry BrakeCue tolerance alignment plus internal unit-authority audit docs; no export names/state IDs, settings, dashboard JSON/layout, marker learning, Pit Entry debrief scoring, `Pit.EntryMargin_m`, or `Pit.EntryRequiredDistance_m` formula changes).
- Audited Pit Entry speed authority: `data.NewData.SpeedKmh` is compared with native session `DataCorePlugin.GameRawData.SessionData.WeekendInfo.TrackPitSpeedLimit`, both treated as kph; the required-distance model converts both speeds to m/s before calculating braking distance. The separate `DataCorePlugin.GameData.PitLimiterSpeed` primary resolver remains a PitExit/PitBox display seam, not Pit Entry braking authority. No SimHub MPH/KPH display setting path or hardcoded 80 kph Pit Entry assumption was found.
- Aligned the limiter-compliant BrakeCue override with the existing Pit Entry line-speed tolerance: limiter ON plus `Pit.EntrySpeedDelta_kph <= +1.0` now reuses existing compliant speed states instead of pre-line `BRAKE NOW` / `BRAKE HARD`. This keeps cue priority consistent with line debrief `normal`/within-margin classification while leaving braking-distance math, marker learning, and debrief scoring unchanged.
- Property Snapshot list reviewed: yes; no SimHub export/property add, remove, rename, state-ID change, or regroup required because only existing `Pit.EntryBrakeCueText` / `Pit.EntryBrakeCueState` selection priority changed within the existing Pit/PitExit surface.

## 2026-06-21 — Pit Entry limiter-compliant BrakeCue priority fix
- Classification: **both** (driver-facing Pit Entry Assist cue correctness plus internal cue-contract docs; no export names/state IDs, braking-distance math, marker learning, `Pit.EntryMargin_m`, `Pit.EntryRequiredDistance_m`, Pit Entry line debrief scoring, dashboard JSON/layout, or settings changes).
- `Pit.EntryBrakeCueText` / `Pit.EntryBrakeCueState` now give limiter ON plus at/below-limit speed priority over pre-line `BRAKE NOW` / `BRAKE HARD`, reusing the existing compliant speed states (`SPEED OKAY`, `BELOW LIMIT`, `TOO SLOW`) once the braking objective is already achieved. Limiter OFF and limiter ON overspeed paths continue to use the existing braking cue selection.
- Property Snapshot list reviewed: yes; no SimHub export/property add, remove, rename, state-ID change, or regroup required because only the existing Pit Entry BrakeCue state-selection priority changed within the existing Pit/PitExit export surface.

## 2026-06-17 — Pit Entry post-line phase split and evidence preservation
- Classification: **both** (Pit Entry/Pit Debrief correctness plus dashboard-facing BrakeCue state-map behavior; no export names, braking-distance math, pit entry line classification, 1.0 kph tolerance, Pit Debrief SummaryText logic, PitCycleLite, Fuel Model, PitExit prediction, or dashboard JSON/layout changes).
- BrakeCue now has an explicit phase split: pre-line remains geometry/braking-only (`READY`, `BRAKE IN Xm`, `BRAKE NOW`, `BRAKE HARD`) until the real pit-entry line is crossed, even after the fake/buffer brake line; post-line hold is speed-settling-only (`SLOW DOWN`, `SPEED OKAY`, `BELOW LIMIT`, `TOO SLOW`) and neutralizes live braking geometry (`Pit.EntryDistanceToLine_m`, `Pit.EntryRequiredDistance_m`, `Pit.EntryMargin_m`, `Pit.EntryCue`) while `Pit.EntryAssistActive` remains true. `Pit.EntryBrakeCueState` now maps `6=SLOW DOWN`, `7=SPEED OKAY`, `8=BELOW LIMIT`, `9=TOO SLOW`.
- The immediate post-line Pit Entry handover reset disables active cue outputs without clearing latched entry-line evidence (`Pit.EntrySpeedDelta_kph`, `Pit.EntryLineLateBy_m`, `Pit.EntryLineDebrief`, `Pit.EntryLineDebriefText`, `Pit.EntryLineTimeLoss_s`, and `Pit.EntryLineDebriefSerial`) before Pit Debrief reads it later in the tick. This preserves current-stop `ENTRY BAD (+x.xkph / y.ym late)` detail even when `Pit.Box.DistanceM < 200m` or pit-box distance authority publishes `0` immediately after crossing.
- Property Snapshot list reviewed: yes; no SimHub export/property add/remove/rename or new snapshot group required, but the `Pit.EntryBrakeCueState` behavior/map remains in the existing Pit/PitExit group through the `Pit.*` prefix.

## 2026-06-17 — Pit Entry BrakeCue text/state refinement and post-line hold
- Classification: **both** (dashboard-facing Pit Entry Assist BrakeCue export refinement plus active-window behavior; no braking-distance math, pit entry line detection/debrief classification, 1.0 kph line-debrief tolerance, Pit Debrief SummaryText, PitCycleLite, Fuel Model, PitExit prediction, or dashboard JSON/layout changes).
- Refined `Pit.EntryBrakeCueText` to the driver-facing vocabulary `OFF`/`FAULT`/`READY`/`BRAKE IN Xm`/`BRAKE NOW`/`BRAKE HARD`/`SLOW DOWN`/`SPEED OKAY`/`BELOW LIMIT`/`TOO SLOW`, removing `BRAKE SOON` and `LATE` from the new BrakeCue text only while preserving legacy `Pit.EntryCue` and `Pit.EntryCueText`. Added `Pit.EntryBrakeCueState` (`0..9`) so dashboards can colour/animate without parsing text.
- `BRAKE IN` now counts down to the configured buffer/fake brake line with `max(0, Pit.EntryMargin_m - Pit.EntryBuffer_m)` and starts no later than 300m before that point. Speed-settling bands use simple ±1 kph hysteresis around +2/-10/-20 kph. Post-line `Pit.EntryAssistActive` remains true after entry crossing until pit-box handover via the existing `Pit.Box.DistanceM < 200m` seam. Property Snapshot list reviewed: yes; `Pit.EntryBrakeCueState` and existing `Pit.EntryBrakeCueText` resolve to the existing Pit/PitExit group through `ResolvePropertySnapshotGroup(...)`, with no new group required.

- 2026-06-16 Pit Debrief entry summary export alignment landed:
  - Added `Pit.Debrief.Entry.SummaryText` as the plugin-owned entry-only first section of full `Pit.Debrief.SummaryText`; both now refresh from the same entry-section formatter so staged dashboards can show pit-entry debrief text without NCalc reconstruction.
  - Current-stop bad entry evidence now makes `Pit.Debrief.Entry.QualityText=BAD` and `Entry.SummaryText`/full `SummaryText` render matching `ENTRY BAD (+x.xkph / y.ym late)` detail while leaving `Entry.LineTimeLossSec` timing-only (including `0.0` when no positive timing estimate exists) and `Entry.LimiterQualityText=POOR`. Safe/normal entries, including marginal overspeed within +1.0 kph, preserve the existing timing-performance GOOD/NORMAL/POOR mapping and limiter SAFE/NORMAL mapping. No dashboard JSON/layout, Pit Entry Assist cue/braking math, PitCycleLite, Fuel Model, PitExit prediction, `Pit.Box.LastDeltaSec` sign, or box/service/strategy debrief behavior changed.
  - Property Snapshot list reviewed: yes; new `Pit.Debrief.Entry.SummaryText` resolves to the existing Pit/PitExit group through `ResolvePropertySnapshotGroup(...)`, with no new group or UI wording required.
...
- Safe/normal and within-margin (+1.0 kph tolerance) entries keep the existing timing-loss performance headline (`ENTRY GOOD/NORMAL (Δ +x.xs)`). The ENTRY BAD override requires current-stop bad-entry evidence (fresh line verdict plus speed above the +1.0 kph tolerance or meaningful late distance), stale previous-stop bad tokens are ignored, and once current evidence is captured, later pit-entry-assist reset/default refresh values do not overwrite it during collection/finalization. The Pit Entry Assist cue/braking math, box/service/strategy delta behavior, exports, logs, and dashboard layouts are unchanged. Property Snapshot list reviewed: yes; no SimHub exports/properties were added, removed, renamed, behavior-recontracted, or regrouped.
...
- Added an early no-op guard for no-service/no-evidence states and moved `[LalaPlugin:PitDebriefBoxDiag]` / `[LalaPlugin:PitDebriefFuelDiag]` behind the existing `SoftDebugEnabled && Settings.EnableDebugLogging` verbose gate. The final `[LalaPlugin:PitDebrief]` summary remains always-on.
- Property Snapshot list reviewed: yes; no SimHub export/property additions, removals, renames, behavior-contract changes, or regrouping required.

## 2026-06-11 — PitExit selective lower active-cycle refresh follow-up
- Classification: **both** (driver-facing active PitExit countdown/predicted rejoin correctness plus internal docs; no new exports, dashboard JSON, UI controls, learning storage, profile persistence, PitEngine timing, or Property Snapshot grouping changes).
- Added a deliberate pit-service model key from Strategy/profile/runtime service inputs (selected pit regulation, NEC factor, refuel amount/rate, tyre count/time, and modeled overhead) and pass it into Opponents/PitExit with the shared total-stop-loss value. During an active pit cycle, lower total-loss estimates are accepted only when this key changes; unchanged-key decreases remain ignored so repair remaining naturally ageing down cannot double-count elapsed time.
- Higher shared estimates still extend the active countdown/predicted position, and deliberate active service-input reductions (for example Default → IMSA/NEC or lower selected fuel/tyres) can lower `PitExit.RemainingCountdownSec` immediately when safely distinguishable. Property Snapshot list reviewed: yes; no SimHub export/property additions, removals, renames, behavior-contract changes, or regrouping required.
...
- Fixed PitExit active-cycle propagation so distinguishable regulation-aware service model changes can refresh the active total-loss seed during the active cycle; the later selective-refresh follow-up above documents the final guard that lower estimates are accepted only when the pit-service model key changes. `PitExit.RemainingCountdownSec`, predicted position, and ahead/behind gaps recalculate from accepted loss changes during the active cycle. A position can remain unchanged when the revised projected-exit point does not cross another class car.
...
- Lifecycle observation arms only from the existing full `ManualRecoveryReset(...)` reset fanout (covering session token/session type/manual resets that actually reset the traffic chain) and clears each subsystem arm after the first conservative healthy proxy; rows bypass MonitorSystem publication, so dashboard-facing `MonitorSystem.Text` and warning priority remain unchanged. Property Snapshot list reviewed: yes; no export/property add, remove, rename, or regroup required.
...
- Strategy and Preset Manager UIs expose manual regulation selection only; Profile CAR tab now groups pit service/pit assist fields and moves Rejoin Assist into its own group with numeric-friendly refuel/tyre fields. No automatic series/track/car/session/preset-name detection was added. Existing export names and Property Snapshot groups remain unchanged; `Fuel.Live.TotalStopLoss`, `Pit.Box.TargetSec`, `Pit.Box.RemainingSec`, PitExit, Strategy, and PreRace outputs now consume regulation-aware timing through existing seams. Property Snapshot list reviewed: yes; no export additions/removals/renames or grouping change.
...
- `[LalaPlugin:PitDebriefBoxDiag]` target tyre count/evidence now reports the resolved current debrief tyre count before or after the countdown latch. Preserved reset-in-lane stale-evidence clearing, in-service flag clear-down freeze, conservative 4-tyre fallback when current-stop evidence is unavailable, and existing Pit Debrief fuel completion/cancel behavior. Property Snapshot list reviewed: yes; no SimHub exports/properties were added, removed, renamed, behavior-recontracted, or regrouped.
...
- Preserved pre-service tyre-selection updates, confirmed fuel-only `0`-tyre Daytona targets, frozen in-service tyre evidence after countdown/service start, and the conservative 4-tyre fallback when no current-stop evidence is available. Property Snapshot list reviewed: yes; no SimHub exports/properties were added, removed, renamed, behavior-recontracted, or regrouped.

## 2026-06-09 — Pit Debrief same-tick refuel completion evidence follow-up
- Classification: **both** (bounded Pit Debrief service-target behavior correction plus diagnostic field addition; no export names, settings, dashboard JSON/layout, PitCycleLite, fuel model, PitExit prediction, or `Pit.Box.LastDeltaSec` sign change).
- Natural refuel completion/cancel detection now includes the current fuel sample when Pit Debrief refreshes before the fuel gauge update, so a same-tick `_isRefuelSelected` reset after reaching the target window preserves `Pit.Debrief.Service.FuelTargetLitres` instead of latching an explicit cancel from stale `Pit_AddedSoFar` evidence.
- True no-refuel/pre-flow cancels still require positive fuel movement and continue to clear stale targets; diagnostics now report `currentAddedEvidence` alongside the natural-completion decision and completion tolerance.
...
- Current-stop tyre evidence now keeps updating while the car is in pit lane before valid box service/countdown starts, so an initial confirmed `0`-tyre fuel-only selection can become a later non-zero MFD tyre selection before the stall; live in-service DP tyre flag clear-down is still frozen out once service starts, and confirmed pre-service `0`-tyre evidence remains valid for fuel-only stops.
...
- `[LalaPlugin:PitDebriefBoxDiag]` now includes target tyre count/evidence and `[LalaPlugin:PitDebriefFuelDiag]` includes completion tolerance. Property Snapshot list reviewed: yes; no SimHub exports/properties were added, removed, renamed, behavior-recontracted, or regrouped, and existing `Pit.*` / `PitExit.*` properties remain in the Pit/PitExit group.
...
- Classification: **both** (driver-facing Pit Debrief box delta/fuel-target behavior and documentation semantics; no export names, settings, dashboard JSON/layout, PitCycleLite, fuel model, or PitExit prediction changes).
- `Pit.Box.LastDeltaSec` preserves its existing dashboard contract (`target - actual`; positive = quicker/better than target, negative = slower/worse), while Pit Debrief inverts that seam internally for its own summary convention (`actual - predicted`; positive = slower). The post-countdown refresh is keyed by a per-stop completed-delta validity flag that clears at pit-entry/debrief start and next boxed-stop activation, so exact `0.0s` on-target stops are valid without leaking a previous stop's delta. Implausible debrief deltas (`abs(delta) > 10.0s`) remain suppressed.
- `Pit.Debrief.Service.FuelTargetLitres` preserves the largest positive requested-add value observed from existing `Fuel.Pit.Box.WillAddLatched` / `Fuel.Pit.WillAdd` evidence across box-exit reset ticks, but an explicit in-box refuel deselect clears the debrief target to `0.0` so canceled no-refuel stops do not report stale requested litres. Actual added fuel remains separate and is not used as a fake target.
- Pit Entry Assist safety/compliance behavior remains unchanged: `LimiterQualityText=POOR` still means the original line-speed compliance verdict was bad; `EntryQualityText` remains performance-oriented from `Pit.EntryLineTimeLoss_s`, and a zero line loss in a bad compliance path is documented as "no valid positive time-loss estimate", not proof of legal compliance.
- Property Snapshot list reviewed: yes; no exports were added, removed, renamed, or regrouped. Existing `Pit.*` / `PitExit.*` exports remain in the Pit/PitExit group.
...
- Preserved fuel-health priority (`CHECK FUEL DATA` / `FUEL DATA FAULT` block pit/baseline warning publication), `Fuel.Refuel.*`, Pit Fuel Control, Pit Command, Strategy planner, fallback systems, CSV writers, dashboard JSON, exports, and per-tick cadence.
- Property Snapshot list reviewed: yes; no export/property add, remove, rename, behavior-contract change, or group change because `MonitorSystem.*` remains in the existing Fuel/Strategy group.
...
- Narrowed fuel-health blocking to unresolved `CHECK FUEL DATA` / `FUEL DATA FAULT` only, so auto-recoverable `FUEL DATA RECOVERED` no longer suppresses urgent Phase 2B pit warnings.
...
- Added a narrow fuel-health priority guard so Phase 2B pit warnings neither publish over nor clear `CHECK FUEL DATA`, `FUEL DATA FAULT`, or `FUEL DATA RECOVERED`; clean pit edges still clear only active Phase 2B pit-warning texts back to `MONITOR READY`.
...
- Added narrow MonitorSystem ownership detection for active Phase 2B pit-warning texts; relevant clean evaluation edges now clear only `REFUEL OFF`, `MFD FUEL LOW`, or `EXIT FUEL SHORT` back to `MONITOR READY`, leaving `CHECK FUEL DATA`, `FUEL DATA FAULT`, and `FUEL DATA RECOVERED` untouched.
...
- A prior `CHECK FUEL DATA` WATCH now clears to `FUEL HEALTH OK` on the next evaluation that satisfies the existing healthy pass basis, even when the queued-check flag was already cleared; the existing passed log remains queued-check-only.
...
- Mirrored existing fuel-health outcomes only: unhealthy observation -> `CHECK FUEL DATA`, unchanged pass -> `FUEL HEALTH OK`, unchanged recovery success -> `FUEL DATA RECOVERED`, unchanged recovery failure -> `FUEL DATA FAULT`. Existing trigger conditions, health checks, logs, 450 ms evaluation throttle, two-observation streak, two-second recovery throttle, recovery state mutations, fuel math, Strategy/PreRace, `Fuel.Refuel.*`, `Pit.FuelControl.*`, and `Pit.Command.*` remain unchanged.
