...
- **`[LalaPlugin:Tyre Learn] accepted/persisted corrected tyre change time ... (raw ..., method=derived|fixed_tail, ...).`** — Valid all-four raw candidate reached in diagnostic phase; persistence is intentionally disabled because live testing shows raw all-four-clear under-reads full tyre service completion.
