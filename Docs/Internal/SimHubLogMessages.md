...
- **`[LalaPlugin:Strategy] StrategyContingencyAdd1L action fired -> value=<litres>L (previous=<litres>L).`** / **`[LalaPlugin:Strategy] StrategyContingencyRemove1L action fired -> value=<litres>L (previous=<litres>L).`** — Dash Control contingency action adjusted the Strategy planner litres contingency through `FuelCalcs.ContingencyValue`.
- **`[LalaPlugin:Strategy] StrategyContingencyAdd1L action ignored -> contingency mode=laps.`** / **`[LalaPlugin:Strategy] StrategyContingencyRemove1L action ignored -> contingency mode=laps.`** — Dash Control contingency action was ignored because the Strategy planner is currently using lap-based contingency.
