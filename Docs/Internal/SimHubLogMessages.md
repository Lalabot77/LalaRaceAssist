...
- **`[LalaPlugin:PitCommand] action=FuelSetMax ... raw='<raw>' normalized='<normalized>'`** — `Pit.FuelSetMax` alternates transport payload by press (`#fuel +150` then `#fuel 0` then repeat); audit line raw/normalized fields show which phase was sent. Tank-full short-circuit now applies only to MAX phase (ZERO phase still transports).
