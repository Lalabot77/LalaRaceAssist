...
- `[LalaPlugin:PitDebriefBoxDiag]` — verbose Pit Debrief box-delta source trace emitted only when master debug (`SoftDebugEnabled`) and Settings `Enable Debug Logging` are both enabled. When enabled, it is edge-only for credible boxed-service lifecycle evidence: pit-entry/debrief start, confirmed first boxed-service edge, first post-box edge, `Pit.Box.LastDeltaSec` countdown finalization, debrief refresh from the completed countdown seam, and final review if a boxed stop still has pending delta evidence. It is not emitted during normal operation and must not be used as an always-on event stream. Fields include track identity, driver/player track percent, session/lap, pit phase and lane/stall/box booleans, countdown active state, target seconds, resolved current debrief target tyre count/evidence (frozen countdown latch when available, otherwise current-stop tyre evidence/fallback), repair influence label for the target when known (`targetRepairInfluence`), elapsed/final elapsed, `Pit.Box.LastDeltaSec` validity, the debrief delta passed, the exported sign note (`target - actual`, positive quicker) for `Pit.Box.LastDeltaSec`, and a reason string. Debrief Box Delta diagnostics may state that the passed delta came from the repair-excluded debrief target while `Pit.Box.LastDeltaSec` remains repair-aware.
...
- **`[LalaPlugin:MonitorSystem] MonitorSystemToggle -> Enabled=<true|false>`** — SimHub action log emitted when the dashboard/hardware toggle changes the existing Monitor System enabled setting. It does not add a new MonitorSystem output property; `MonitorSystem.Text` reflects `MONITOR READY` or `MONITOR OFF`.
...
- **Monitor Event CSV `Lifecycle` rows (`OPPONENTS RESTARTED`, `CARSA RESTARTED`, `H2H RESTARTED`, optional `LAPREF RESTARTED`)** - Debug-only CSV evidence rows written through the existing `MonitorSystem_Events.csv` writer when enabled. They are one-shot first-healthy-after-reset observations armed by the existing full lifecycle/manual reset fanout, bypass MonitorSystem publication, and do not change `MonitorSystem.Text`, warning priority, exports, MSGV1, or reset/recovery/calculation behavior.
...
- **`[LalaPlugin:MsgCx] MsgCx action fired (pressed latched + engines notified + pit fuel fault recovery checked).`** — `MsgCx` / Acknowledge action invoked; message engines receive the acknowledge/dismiss signal and Pit Fuel Control performs its no-command stale pending-request recovery check.
...
- **Registered SimHub action surface (summary)** — Current plugin-owned action families include Acknowledge (`MsgCx`), pit screen, pit commands, Pit Fuel Control (`SourceCycle` / `ModeCycle` / `SetPush` / `SetNorm` / `SetSave` / DATA actions / `SetPlan` compatibility), Pit Tyre Control (`ModeCycle` plus direct `SetOff` / `SetDry` / `SetWet` / `SetAuto`), `CustomMessage01..10`, compatibility aliases `Pit.FuelAdd` / `Pit.FuelRemove`, `PrimaryDashMode` Plugin Quick Reset, `StrategyDash.ModeToggle`, burn-analysis actions, `MonitorSystemToggle`, `LeagueClass.ToggleEnabled`, declutter/dark-mode/dark-mode-brightness/Strategy contingency/event-marker/property-snapshot actions, launch, track-marker / track-learning lock actions, and Shift Assist actions. `Pit.FuelControl.SetPlan` is a compatibility alias for the DATA model; legacy `Pit.FuelControl.PushSaveModeCycle` and `SecondaryDashMode` are not registered.
...
- **`[LalaPlugin:Dash] DarkModeBrightnessDecrease action fired -> value=<new> (previous=<old>).`** / **`[LalaPlugin:Dash] DarkModeBrightnessIncrease action fired -> value=<new> (previous=<old>).`** — Dash Control brightness actions adjusted the persisted Dark Mode brightness slider value.
- **`[LalaPlugin:ShiftAssist] ShiftAssistToggle action fired -> Enabled=<true|false>`** — Dash Control Shift Assist action toggled only the Shift Assist enabled setting.
- **`[LalaPlugin:Strategy] StrategyContingencyAdd1L action fired -> value=<litres>L (previous=<number>, switchedToLitres=<true|false>).`** / **`[LalaPlugin:Strategy] StrategyContingencyRemove1L action fired -> value=<litres>L (previous=<number>, switchedToLitres=<true|false>).`** — Dash Control contingency action ensured Strategy contingency is in litres mode when needed, then adjusted the planner contingency through `FuelCalcs.ContingencyValue`.
...
- **`[LalaPlugin:Launch Trace] Removed empty launch trace file: <path>`** — Verbose-debug housekeeping removed an obviously empty historical launch trace during Standing Start Review file-list discovery so it is not shown as selectable.【F:LalaLaunch.cs†L15159-L15169】
...
Emitted once when a pit debrief finalizes after the existing out-lap/final pit-loss seam. This is a structured final readout for the latest completed stop and must not repeat while `Pit.Debrief.Valid` remains true. `actualLossSec` is total-stop-equivalent: current DTL/direct lane-equivalent final loss plus latched stationary box duration for boxed stops. Unavailable numeric fields use `na`; exported numeric fields publish safe `0` defaults. Exit prediction evidence remains in the structured fields; the quoted `summary` intentionally omits exit wording and uses progressive-summary wording latched at finalization. The log includes `rawBoxDeltaSec` and retained compatibility field `boxDeltaSuppressed` (expected `false` for finite valid deltas) so pending `BOX ... (Δ PENDING)` can be audited without per-tick logging; box delta uses Pit Debrief sign (`repair-adjusted driver-controlled elapsed - repair-excluded predicted service target`, positive slower). It is no longer the inverse of the repair-aware dashboard-facing `Pit.Box.LastDeltaSec`; pending means missing, invalid, or not-completed source only; preliminary box-entry target/elapsed evidence is not treated as completed evidence. `entryLossSec` is printed with two decimals for debug review, while SimHub numeric exports remain raw doubles.
