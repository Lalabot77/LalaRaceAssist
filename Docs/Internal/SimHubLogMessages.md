...
- **`[LalaPlugin:PitCommand] action=FuelSetMax ... raw='<raw>' normalized='<normalized>'`** — `Pit.FuelSetMax` now alternates transport payload by press (`#fuel +150` then `#fuel 0` then repeat); audit line raw/normalized fields show which phase was sent.
