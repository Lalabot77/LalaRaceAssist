...
- **`[LalaPlugin:MonitorSystem] car health check=<OppTarget|CarSASlot|CarSAGap|H2HTarget> status=<fail|still-failing|recovered> text=<OPP TARGET CHECK|CARSA SLOT CHECK|CARSA GAP CHECK|H2H TARGET CHECK> sessionTime=... sessionType=... playerCarIdx=... reason=...`** - Phase 3A Car/Opp/H2H impossible-state evidence log. Fail/recovery logs are edge-triggered; repeated still-failing logs are log-only and rate-limited. The checks observe existing published/internal Opponents, CarSA, and H2H outputs only and do not recalculate gaps, validate race order/checkpoint freshness, select fallbacks, reset subsystems, send commands, write CSV, or add SimHub exports.
