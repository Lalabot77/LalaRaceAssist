...
- **`[LalaPlugin:Leader Lap] reject source=overall_p1_last_lap|overall_p1_best_lap_low_conf ... reason=...`** — Overall leader candidate rejected by absolute plausibility only (invalid, too small, or too large); may fall back to existing rolling average when available.
- **`[LalaPlugin:Leader Lap] holding overall leader rolling avg reason=... completed_player_lap=... avg_s=...`** — Temporary missing/unsampleable overall-P1 feed is using the bounded held rolling average for projection continuity.
- **`[LalaPlugin:Leader Lap] held overall leader pace expired reason=... current_player_lap=... last_valid_player_lap=... – returning 0`** — One-completed-player-lap hold expired without fresh overall-P1 authority; leader lap fails closed.
- **`[LalaPlugin:Leader Lap] no valid overall leader ... – returning 0`** — Overall P1 identity or pace authority unavailable and no bounded hold/fallback is available; leader lap fails closed.
