...
- **`[LalaPlugin:PitCommand] action=<Action> transport=<postmessage|sendinput> attempted=true confirmed=<true|false> before=<before> raw='<raw>' normalized='<normalized>'`** — Action fire audit line for pit commands; for stateful commands, `confirmed` (before/after check) remains the authoritative outcome.
...
- **`[LalaPlugin:PitCommand] custom-message transport=<postmessage|sendinput> attempted=true text='<text>'`** — Custom-message send audit line when transport was attempted.
...
- **`[LalaPlugin:PitCommand] raw-command transport=<postmessage|sendinput> attempted=true raw='<raw>' normalized='<normalized>'`** — Raw pit-command send audit line for paths that intentionally reuse built-in pit-command normalization before injection.
