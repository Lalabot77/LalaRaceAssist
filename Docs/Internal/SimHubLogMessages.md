...
- **`[LalaPlugin:PitCommand] transport=sendinput unavailable reason=not-foreground.`** — One-time warning when legacy foreground `SendInput` cannot run because iRacing is not foreground.
- **`[LalaPlugin:PitCommand] transport=sendinput local submission issue while sending command text (transport attempt unconfirmed).`** — One-time warning when legacy unicode `SendInput` text submission reports a local issue.
- **`[LalaPlugin:PitCommand] transport=sendinput fallback_from=postmessage reason=<reason>`** — Info line showing Auto mode fallback from direct window-message transport to legacy sendinput (`reason` examples: `no-iracing-process`, `no-iracing-window`, `postmessage-char-failed`).
- **`[LalaPlugin:PitCommand] transport=sendinput fallback_suppressed=true reason=postmessage-partial-state-unsafe`** — Warn line when Auto mode intentionally suppresses legacy fallback because direct `postmessage` already mutated chat state (prevents second-path corruption/duplicate-open behavior).
...
- **`[LalaPlugin:PitCommand] action=<Action> transport=<postmessage|sendinput|none> local-transport-issue reason=<reason> [fallback_from=postmessage] raw='<raw>' normalized='<normalized>'`** — Best-effort transport-stage issue before state confirmation; action publishes `Pit Cmd Fail`.
- **`[LalaPlugin:PitCommand] action=<Action> expected-state-mismatch expected=<bool> before=<before> after=<after> transport=<postmessage|sendinput>`** — Stateful command did not produce expected before/after toggle confirmation; action publishes `Pit Cmd Fail`.
- **`[LalaPlugin:PitCommand] action=<Action> transport=<postmessage|sendinput> attempted=true delivery=<verified|unverified> effect-confirmed=<true|false> before=<before> raw='<raw>' normalized='<normalized>'`** — Action fire audit line for pit commands. Stateful built-ins use before/after telemetry as authoritative effect confirmation (`effect-confirmed=true` only when confirmed); stateless built-ins remain transport-attempt only (`delivery=unverified`, `effect-confirmed=false`).
...
- **`[LalaPlugin:PitCommand] custom-message transport=<postmessage|sendinput|none> local-transport-issue reason=<reason> [fallback_from=postmessage] text='<text>'`** — Custom-message send hit a local transport-stage issue; action publishes `Pit Cmd Fail`.
- **`[LalaPlugin:PitCommand] custom-message transport=<postmessage|sendinput> attempted=true delivery=unverified effect-confirmed=false text='<text>'`** — Custom-message send audit line when transport was attempted; this is transport-attempt observability only (no authoritative in-sim effect confirmation seam).
...
- **`[LalaPlugin:PitFuelControl] PitFuelControlModeCycle action received`** — `LalaLaunch` action entry marker proving SimHub binding reached plugin method for ModeCycle (same pattern used for SourceCycle/SetPush/SetNorm/SetSave/SetPlan).
- **`[LalaPlugin:PitFuelControl] entry action=<ModeCycle|SourceCycle|SetPush|SetNorm|SetSave|SetPlan|OnLapCross> mode=<OFF|MAN|AUTO> source=<PUSH|NORM|SAVE|PLAN|STBY> autoArmed=<bool> isAutoModeActive=<bool> suppressFuelControl=<bool> iracingAutoFuelEnabled=<bool> telemetryFuelFillEnabled=<bool> planValid=<bool|n/a> targetLitres=<value|n/a> overrideActive=<bool> lastSentFuelLitres=<value|none>`** — Fuel Control engine entry snapshot emitted on button/action entry and gated lap-cross AUTO evaluation paths.
- **`[LalaPlugin:PitFuelControl] blocked action=<...> reason=<snapshot-null|suppressed|off-hard-guard|auto-plan-blocked|plan-invalid|source-stby|target-invalid|send-failed|auto-not-armed|lap-cross-no-material-delta|iracing-autofuel-ownership|external-mirror-change|owned-mirror-consumed>`** — Explicit no-send/early-return diagnostics for Fuel Control action and ownership paths (`INFO` for user/action-relevant blocks, `DEBUG` for cadence-heavy no-op detail).
...
- **`[LalaPlugin:Init] Actions registered: MsgCx, TogglePitScreen, Pit.ClearAll, Pit.ClearTires, Pit.ToggleFuel, Pit.FuelSetZero, Pit.FuelAdd1, Pit.FuelRemove1, Pit.FuelAdd10, Pit.FuelRemove10, Pit.FuelSetMax, Pit.ToggleTiresAll, Pit.ToggleFastRepair, Pit.ToggleAutoFuel, Pit.Windshield, Pit.FuelControl.SourceCycle, Pit.FuelControl.ModeCycle, Pit.FuelControl.SetPush, Pit.FuelControl.SetNorm, Pit.FuelControl.SetSave, Pit.FuelControl.SetPlan, CustomMessage01..CustomMessage10 (+ aliases Pit.FuelAdd/Pit.FuelRemove), PrimaryDashMode, DeclutterMode, ToggleDarkMode, SecondaryDashMode (legacy), EventMarker, LaunchMode, TrackMarkersLock, TrackMarkersUnlock`** — Init-time action registration summary for SimHub bindings.
