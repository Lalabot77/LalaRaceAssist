...
- **`[LalaPlugin:PitTyreControl] Manual truth sync remap reason=<manual-confirmation-fallback|manual-external-truth-sync> mode=<OFF|DRY|WET>`** — Outside AUTO, manual tyre mode was remapped to actual MFD truth after bounded confirmation/reconciliation (prevents stale OFF/DRY/WET intent display).
