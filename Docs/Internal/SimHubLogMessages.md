...
- **Registered SimHub action surface (summary)** — Current plugin-owned action families include Acknowledge (`MsgCx`), pit screen, pit commands, Pit Fuel Control (`SourceCycle` / `ModeCycle` / `SetPush` / `SetNorm` / `SetSave` / DATA actions / `SetPlan` compatibility), Pit Tyre Control (`ModeCycle` plus direct `SetOff` / `SetDry` / `SetWet` / `SetAuto`), `CustomMessage01..10`, compatibility aliases `Pit.FuelAdd` / `Pit.FuelRemove`, `PrimaryDashMode` Plugin Quick Reset, `StrategyDash.ModeToggle`, burn-analysis actions, `MonitorSystemToggle`, `LeagueClass.ToggleEnabled`, declutter/dark-mode/event-marker/property-snapshot actions, launch, track-marker / track-learning lock actions, and Shift Assist actions. `Pit.FuelControl.SetPlan` is a compatibility alias for the DATA model; legacy `Pit.FuelControl.PushSaveModeCycle` and `SecondaryDashMode` are not registered.
