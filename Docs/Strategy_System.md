...
- PreRace fuel-needed basis uses the matching Strategy planner total when planner/live car, track, race basis, race length, and any manually forced wet/dry condition agree; it dynamically replaces planned formation fuel with remaining formation fuel. If those gates do not pass, PreRace keeps its live/session fallback calculation rather than trusting a mismatched plan.
