...
- `LalaLaunch.Pit.FuelControl.ModeCycle` now uses explicit raw fuel command ownership (no internal fuel-toggle semantics):
  - `OFF -> MAN` is selection/intent only (`FUEL MAN STBY`) and sends no command,
  - `MAN -> AUTO` enters plugin AUTO ownership,
  - `AUTO -> OFF` attempts explicit `#-fuel$`; successful send exits AUTO, failed send falls back to `AUTO STBY`.
...
- `SetPlan` is MAN-only direct PLAN send; PLAN is blocked in OFF/AUTO.
