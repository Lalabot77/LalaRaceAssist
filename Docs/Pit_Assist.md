...
Pit Debrief box/fuel source-trace diagnostics are SimHub log-only and do not require dashboard JSON/layout updates. Existing `Pit.Debrief.*` exports keep their names; `Pit.Box.LastDeltaSec` keeps its dashboard sign contract (`target - actual`, positive quicker/better). Box targets use the current-stop latched tyre count, so confirmed fuel-only stops do not carry stale/default 4-tyre timing; the 4-tyre fallback is retained only when tyre evidence is unavailable. `Pit.Debrief.Service.FuelTargetLitres` remains debug/readout-only, preserves positive requested-add evidence through normal refuel completion/reset or deselect when added fuel is within the completion-tolerance window, and still clears explicit in-box refuel-cancel before natural completion or true no-refuel/pre-flow cancel cases.
