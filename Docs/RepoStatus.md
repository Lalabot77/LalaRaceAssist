- 2026-07-09 Runtime dashboard manifest filename alignment landed:
  - manifest runtime `file` values now match the current exported/imported root `.djson` names for Driver Dash, Strategy Dash, Alerts Overlay, Vertical Traffic Bar Overlay, and Fuel Calculator while preserving `folder`, `latest`, release-audit metadata, scanner logic, Overview UI, dashboard packages/JS, and plugin exports. Overview installed-package detection should now find those imported dashboards through the existing deterministic `DashTemplates/<folder>/<file>` scan. Property Snapshot list reviewed: no; manifest filename metadata only with no export/property surface change.
- 2026-07-09 VersionManifestAudit exported dashboard package support landed:
  - release audits now validate official `Dash Files/*.simhubdash` export packages directly using manifest `releaseAudit.package` / `releaseAudit.rootFile` metadata while preserving runtime `folder` / `file` / `versionProperty` fields for installed SimHub `DashTemplates` detection. The audit intentionally reports the current Driver Dash package `v1.1.9` vs manifest `1.1.0` mismatch; runtime installed detection, embedded manifest loading, Overview UI, dashboard files/JS, and plugin exports are unchanged. Property Snapshot list reviewed: yes; no export names or snapshot groups changed.
...
- 2026-06-27 v1.1 final documentation consistency pass landed:
  - fixed stale moved-document references, including the Pit System pit-loss planner reference to `Docs/Subsystems/Strategy/Fuel_Planner_Tab.md`, restored concise README/Quick Start/User Guide migration guidance for `LaunchPlugin.dll`, `RSC.iRacingExtraProperties.dll`, plugin-owned pit/custom actions, and legacy binding review, and aligned landing-page terminology.
  - status docs now distinguish the documentation architecture/content work from the visible user-facing UI/product wording updates: runtime behavior, dashboard JSON/package files, SimHub exports/actions, settings schema, profile/preset schema, and dashboard contracts remain unchanged. Property Snapshot list reviewed: no; documentation consistency and migration guidance only, with no SimHub export/property surface change.
- 2026-06-27 v1.1 documentation philosophy polish landed:
  - rewrote the Core System pages and User Guide into more natural guidance-oriented introductions while keeping the completed documentation architecture, file paths, runtime behavior, dashboard JSON/package files, exports/actions, schemas, and contracts unchanged.
  - User Guide now explains product surfaces, driving versus race management, core-system flow, learn/validate/lock/trust philosophy, ownership boundaries, and how to continue through Product/Systems/Features/Subsystems/Internal docs. Property Snapshot list reviewed: no; content-only documentation polish with no SimHub export/property surface change.
...
- 2026-06-27 v1.1 documentation architecture restructure landed:
  ...
  - documentation architecture change plus later visible label wording updates; no runtime behavior, dashboard JSON, SimHub export/action names, settings schema, profile schema, preset schema, or images changed. Property Snapshot list reviewed: no; documentation structure and links only, with no SimHub export/property surface change.
...
- 2026-06-26 Pit Debrief Box Delta repair-adjusted edge follow-up landed:
  - completed boxed-stop elapsed is now latched with `Pit.Box.LastDeltaSec` finalization so Pit Debrief can still resolve repair-adjusted Box Delta after live countdown reset when native final duration is briefly invalid;
  - Pit Debrief repair adjustment now reads mandatory plus optional repair-left telemetry through a debrief-only path independent of the runtime optional-repair countdown display setting. Runtime `Pit.Box.TargetSec`, `Pit.Box.RemainingSec`, `Pit.Box.LastDeltaSec`, PitExit, Fuel.Live.TotalStopLoss, Strategy timing, PitEngine/PitCycleLite, dashboard JSON, and settings are unchanged. Property Snapshot list reviewed: yes; no export/property add/remove/rename/regroup required because existing Pit Debrief semantics only were repaired.

- 2026-06-26 first-lap boxed-stop stall timing follow-up landed:
  - The Road America phase handoff path now also runs the existing pit-stall timer edge handling before the first-lap/pre-race pit-loss suppression return. First-lap/pre-race boxed stops that legitimately publish `InBox` keep `PitStopElapsedSec` live/non-zero for active countdown consumers, while drive-through phase progression and pit-loss learning suppression remain unchanged.
  - No dashboard JSON, export names, Pit Debrief semantics, marker learning, pit-box distance math, fuel/refuel/strategy calculations, or settings changed. Property Snapshot list reviewed: yes; no export/property add/remove/rename/regroup required.
- 2026-06-26 Road America pit phase/Box Entry handoff fix landed:
  - PitEngine now keeps live pit phase and Pit Entry Assist handover updates running during first-lap/pre-race pit-road passes where pit-loss learning remains intentionally suppressed. This prevents a pre-line `EnteringPits` phase from staying latched through pit lane and blocking Box Entry visibility when valid forward-wrapped `Pit.Box.DistanceM` evidence exists, including Road America-style pit boxes near/after start-finish.
  - Pit Entry post-line hold, the `<200m` handover seam, wrapped pit-box distance, drive-through baseline learning semantics, boxed-service/debrief semantics, dashboard JSON, and SimHub export names remain unchanged. Property Snapshot list reviewed: yes; no export/property 
- 2026-06-26 Dash Control action follow-up fixes landed:
  - `ShiftAssistToggle` now notifies the Profiles ViewModel checkbox state after changing the persisted Shift Assist enabled setting, while the legacy alias uses the same seam;
  - Strategy contingency +/- now always switches/uses Extra Litres mode before applying +/-1.0 L and clamping to `0.0..20.0`; the old laps-mode ignored/no-op behavior was removed from code/docs/log inventory. Dark Mode brightness +/- remains slider-backed and unchanged. Property Snapshot list reviewed: yes; no SimHub exports/properties changed.
- 2026-06-26 Dash Control missing action bindings landed:
  - added SimHub/dashboard actions for Dark Mode brightness +/- (existing `Settings.DarkModeBrightnessPct`, +/-5, `0..100`), Shift Assist system enable toggle (`Settings.ShiftAssistEnabled` only), and Strategy contingency +/-1L (switches to litres via `FuelCalcs.IsContingencyLitres`, then adjusts `FuelCalcs.ContingencyValue`);
  - exposed the actions in `Bindings -> General / Dash Controls`; no dashboard JSON, fuel formulas, Shift Assist learning/locks/samples, launch logic, or SimHub exports/properties changed; Profiles UI now receives the Shift Assist enabled-state notification when the dashboard action fires. Property Snapshot list reviewed: yes; no export/property changes.
- 2026-06-26 Pit Debrief Box Delta repair-exclusion follow-up landed:
  - Pit Debrief Box Delta now compares repair-adjusted driver-controlled boxed elapsed time against a debrief-only fuel/tyre/boxed-overhead service target that excludes optional and mandatory repair duration, so leaving before optional repairs complete no longer creates a misleading large negative Box Delta and waiting for served repairs no longer creates a misleading positive Box Delta.
  - Repair-aware `Pit.Box.TargetSec`, `Pit.Box.RemainingSec`, `Pit.Box.LastDeltaSec`, PitExit prediction, `Fuel.Live.TotalStopLoss`, Strategy timing, pit-loss learning, and service regulation modelling remain unchanged. Service/repair labels still provide repair context in `SummaryText`. Property Snapshot list reviewed: yes; no export/property add/remove/rename/regroup required because this changes only existing Pit Debrief semantics.
...
- 2026-06-21 Box Entry STOP cue comfort buffer landed:
  - `Pit.Box.BrakeNow` now applies a cue-only comfort buffer derived from the existing Pit Entry Buffer: `clamp(PitEntryBuffer_m * 0.20, 1.0, 5.0)` after the same 0–50 m Pit Entry Buffer sanitization. Example: 15 m Pit Entry Buffer -> 3 m Box STOP cue buffer.
  - No new settings/exports/state IDs, dashboard JSON/layout edits, pit-box distance/time changes, pit-box authority/location changes, stationary detection changes, service timing changes, Pit Entry math changes, PitCycleLite/PitExit changes, `Pit.Box.LastDeltaSec` changes, or Pit Debrief calculation changes. Property Snapshot list reviewed: yes; existing `Pit.Box.BrakeNow` remains in Pit/PitExit.
- 2026-06-21 Pit Entry pit-speed unit audit and BrakeCue tolerance alignment landed:
  - Pit Entry speed authority was audited as `data.NewData.SpeedKmh` versus native session `DataCorePlugin.GameRawData.SessionData.WeekendInfo.TrackPitSpeedLimit`, both treated as kph and converted to m/s for required-distance math; no SimHub display-unit corruption path or hardcoded 80 kph Pit Entry assumption was found.
  - Limiter-compliant BrakeCue priority now aligns with the existing +1.0 kph Pit Entry line-speed tolerance, reusing existing compliant speed states instead of `BRAKE NOW` / `BRAKE HARD` when limiter ON and `Pit.EntrySpeedDelta_kph <= +1.0`. No exports/state IDs, marker learning, `Pit.EntryMargin_m`, `Pit.EntryRequiredDistance_m`, debrief scoring, dashboard JSON/layout, settings, or Property Snapshot grouping changed. Property Snapshot list reviewed: yes.
- 2026-06-21 Pit Entry limiter-compliant BrakeCue priority fix landed:
  - When the limiter is ON and the car is already at/below pit speed, `Pit.EntryBrakeCueText` / `Pit.EntryBrakeCueState` reuse existing compliant speed states (`SPEED OKAY`, `BELOW LIMIT`, `TOO SLOW`) instead of continuing pre-line `BRAKE NOW` / `BRAKE HARD` guidance.
  - No export names/state IDs, braking-distance math, marker learning, `Pit.EntryMargin_m`, `Pit.EntryRequiredDistance_m`, Pit Entry line/debrief scoring, dashboard JSON/layout, settings, or Property Snapshot grouping changed. Property Snapshot list reviewed: yes; existing Pit Entry BrakeCue exports remain in the Pit/PitExit group.
- 2026-06-17 Pit Entry post-line phase split and evidence preservation landed:
  - BrakeCue now stays in pre-line braking mode (`READY`, `BRAKE IN Xm`, `BRAKE NOW`, `BRAKE HARD`) until the real pit-entry line is crossed, then switches to post-line speed-settling-only text/state (`SLOW DOWN`, `SPEED OKAY`, `BELOW LIMIT`, `TOO SLOW`) while neutralizing live braking geometry so dashboards gated by `Pit.EntryAssistActive` do not render stale margin/marker guidance. `Pit.EntryBrakeCueState` now maps post-line speed states as `6=SLOW DOWN`, `7=SPEED OKAY`, `8=BELOW LIMIT`, `9=TOO SLOW`.
  - The immediate post-line Pit Entry handover reset turns off active cue outputs without clearing latched entry-line evidence before Pit Debrief consumes it later in the tick, preserving `ENTRY BAD (+x.xkph / y.ym late)` detail and `Pit.Debrief.Entry.QualityText=BAD` when `Pit.Box.DistanceM < 200m` or pit-box distance authority reports `0` immediately after the line.
  - No export names, braking-distance math, pit entry line classification, 1.0 kph line-debrief tolerance, Pit Debrief SummaryText logic, PitCycleLite, Fuel Model, PitExit prediction, dashboard JSON/layout, or Property Snapshot group changed. Property Snapshot list reviewed: yes; the BrakeCueState behavior/map remains in the existing Pit/PitExit group.
- 2026-06-17 Pit Entry BrakeCue text/state refinement and post-line hold landed:
  - `Pit.EntryBrakeCueText` is now the primary driver-facing BrakeCue text with exact vocabulary `OFF`, `FAULT`, `READY`, `BRAKE IN Xm`, `BRAKE NOW`, `BRAKE HARD`, `SLOW DOWN`, `SPEED OKAY`, `BELOW LIMIT`, and `TOO SLOW`; it no longer emits `BRAKE SOON` or `LATE`. Legacy `Pit.EntryCue` / `Pit.EntryCueText` remain unchanged for compatibility.
  - Added `Pit.EntryBrakeCueState` (`0=OFF`, `1=FAULT`, `2=READY`, `3=BRAKE IN`, `4=BRAKE NOW`, `5=BRAKE HARD`, `6=SLOW DOWN`, `7=SPEED OKAY`, `8=BELOW LIMIT`, `9=TOO SLOW`) for dashboard colours/animations. `BRAKE IN` counts down to the configured buffer/fake brake line via `max(0, Pit.EntryMargin_m - Pit.EntryBuffer_m)` and starts no later than 300m out; speed bands use simple ±1 kph hysteresis.
  - `Pit.EntryAssistActive` now remains true after pit-entry crossing for speed-settling text until handover to the existing Pit Box seam when `Pit.Box.DistanceM < 200m`. No braking-distance math, pit entry line detection/debrief classification, 1.0 kph line-debrief tolerance, Pit Debrief SummaryText, PitCycleLite, Fuel Model, PitExit prediction, or dashboard JSON/layout behavior changed. Property Snapshot list reviewed: yes; new `Pit.EntryBrakeCueState` resolves to the existing Pit/PitExit group through `ResolvePropertySnapshotGroup(...)`, with no new group required.
...
- 2026-06-16 Pit Debrief bad-entry summary compliance override landed:
  ...
  - `safe`/`normal` entry tokens, including marginal overspeed within the existing +1.0 kph tolerance, keep the timing-loss performance headline. The ENTRY BAD override requires current-stop bad-entry evidence (fresh line verdict plus speed above the +1.0 kph tolerance or meaningful late distance), stale previous-stop bad tokens are ignored, and once current evidence is captured, later pit-entry-assist reset/default refresh values do not overwrite it during collection/finalization. No dashboard JSON/layout edits, export additions/removals/renames, PitCycleLite, Fuel Model, PitExit prediction, `Pit.Box.LastDeltaSec` sign, or box/service/strategy delta behavior changes.
...
- Pit service regulation runtime propagation follow-up is implemented without new exports. `Fuel.Live.RefuelRate_Lps` now publishes the active effective pit-service rate through the shared `PitServiceTimeModel` seam (Default/IMSA = normal baseline, NEC = baseline × car-profile NEC percent) while stored `CarProfile.RefuelRate` remains the normal physical/profile value.
- PitExit active-cycle prediction now refreshes its total-loss seed on shared loss changes instead of only accepting increases, so NEC factor/regulation changes propagate to `PitExit.RemainingCountdownSec`, `PredictedPositionInClass`, and ahead/behind gap recalculation. No dashboard property names were added/removed/renamed; PR #827-style `Fuel.Live.PitServiceRefuelRate_Lps` / `Fuel.Live.PitServiceRefuelTime_S` exports are not present. Property Snapshot list reviewed: yes; existing `Fuel.Live.*` remains Fuel/Strategy and `PitExit.*` remains Pit/PitExit, with no group changes required.
...
- Strategy and Preset Manager UIs expose manual regulation selection only; Profile CAR tab groups pit service/pit assist fields with a constrained Base Tank textbox, narrow aligned Refuel Rate / NEC factor / Tyre Change Time editors, two-decimal Refuel Rate display, and full-width Pit Entry Decel/Buffer sliders at the bottom while keeping Rejoin Assist visually separate. No automatic series/track/car/session/preset-name detection was added. Existing export names and Property Snapshot groups remain unchanged; `Fuel.Live.TotalStopLoss`, `Pit.Box.TargetSec`, `Pit.Box.RemainingSec`, PitExit, Strategy, and PreRace outputs now consume regulation-aware timing through existing seams. Property Snapshot list reviewed: yes; no export additions/removals/renames or grouping change.
...
- Pit Debrief PR #818 review follow-up is validated in code/docs: first box-entry target/elapsed evidence no longer creates a debrief box delta, so `BOX ... (Δ PENDING)` remains until completed `Pit.Box.LastDeltaSec`/valid completed evidence is available. Large finite completed deltas still display; NaN/infinite/unavailable/not-completed sources remain pending.
- Repair influence labels are preserved when repair-left telemetry becomes positive after `_pitBoxTargetLatched` and governs `Pit.Box.RemainingSec`; target construction remains unchanged. No export names, settings, dashboard JSON/layout, PitCycleLite, fuel model, PitExit prediction, or `Pit.Box.LastDeltaSec` sign changed. Property Snapshot list reviewed: yes; no group mapping change required.

- Pit Debrief repair-aware box-delta behavior is validated in code/docs: any finite valid `Pit.Box.LastDeltaSec` is displayed in Pit Debrief regardless of magnitude, after inversion to the debrief sign (`actual - target`; positive slower, negative faster). `BOX ... (Δ PENDING)` now means the completed source is missing/invalid, not too large.
- `Pit.Box.TargetSec` remains Option A service-time authority: selected/active fuel, tyres, mandatory repairs, and optional repairs when selected/active/telemetry-reported as part of service are included. Repair-influenced box targets label only the `SummaryText` box section as `BOX MAND REPAIR`, `BOX OPT REPAIR`, or `BOX REPAIRS` while `Pit.Debrief.Box.QualityText` remains the quality enum; optional repair abandonment can produce a large negative debrief delta. No target construction, export names, settings, dashboard JSON/layout, PitCycleLite, fuel model, PitExit prediction, or `Pit.Box.LastDeltaSec` sign changed. Property Snapshot list reviewed: yes; no group mapping change required.
...
- 2026-06-09 Pit Debrief explicit refuel-cancel preservation follow-up landed:
  ...
  - `FuelAddedLitres` remains independent actual-added evidence, `[LalaPlugin:PitDebriefFuelDiag]` reports explicit-cancel/natural-completion/action fields, and no exports/settings/dashboard JSON/layout/PitCycleLite/fuel model/PitExit prediction paths changed. Property Snapshot list reviewed: yes; no group mapping change required.

- 2026-06-08 Pit Debrief Daytona box/fuel-target diagnostics landed:
  - added edge-only `[LalaPlugin:PitDebriefBoxDiag]` logs for pit-entry/debrief start, first box edge, first post-box edge, countdown finalization, completed-delta refresh, and final pending review so Daytona can be compared against Imola/Algarve without per-tick spam;
  - added edge-only `[LalaPlugin:PitDebriefFuelDiag]` logs for existing `Fuel.Pit.*` source seams and the value passed to Pit Debrief at pit entry, box entry, first active service refresh, positive target observation, refuel deselect, box exit, and final debrief;
  - bounded fix: `Pit.Debrief.Service.FuelTargetLitres` preserves positive requested-add evidence through normal refuel completion/reset after natural completion, while explicit in-box refuel deselect before natural completion and true no-refuel/pre-flow cancel stops clear to `0.0`. Diagnostic logs now state whether the clear was an explicit cancel or a normal completion/reset preserve. `Pit.Box.LastDeltaSec` remains `target - actual` (positive quicker/better). No exports/settings/dashboard JSON/layout/PitCycleLite/fuel model/PitExit prediction paths changed. Property Snapshot list reviewed: yes; no group mapping change required.
...
- 2026-06-08 Pit Debrief box delta/fuel-target follow-up landed:
  ...
  - `Pit.Debrief.Service.FuelTargetLitres` now latches the largest positive existing requested-add evidence from `Fuel.Pit.Box.WillAddLatched` / `Fuel.Pit.WillAdd` across box-exit reset ticks; actual added fuel remains separate; later follow-up narrowed in-box refuel-deselect clearing so normal refuel completion/reset does not erase positive target/add evidence, while explicit in-box cancel still clears the target;
  - Pit Entry Assist safety/compliance thresholds and line logic were reviewed unchanged: `LimiterQualityText=POOR` can coexist with `EntryQualityText=GOOD` when the line-speed compliance verdict is bad but the performance time-loss export is zero/no-positive-estimate. No PitCycleLite, fuel model, PitExit prediction, dashboard JSON/layout, settings, export names, or Property Snapshot grouping changed. Property Snapshot list reviewed: yes; existing `Pit.*`/`PitExit.*` exports remain in the Pit/PitExit group.
...
- 2026-06-06 CarSA reverse checkpoint matching for ahead precision availability landed:
  - player checkpoint crossings now reverse-match recent eligible non-player timestamps already recorded for the same gate, so normal ahead-order crossings can populate the existing gate truth/filter cache after the player arrives;
  - reverse matches keep the existing stored sign and lap normalization, enforce a reverse-only `|target lap - player lap| <= 2` bound, require current on-track/non-pit eligibility, reject future/stale timestamps beyond `min(15 s, half the active lap-time scale)`, and skip same-update target crossings to avoid duplicate truth/rate observations;
  - CarSA slot selection, `Gap.TrackSec`, `Gap.RelativeSec` mapping/sticky/fallback/source behavior, precision sign/tolerance/freshness, the Opponents direct checkpoint seam, H2HTrack, H2HRace, Opponents, PitExit, dashboard JSON, and export names remain unchanged;
  - Property Snapshot list reviewed: yes; no group change because no exports are added, removed, renamed, or regrouped.
...
- 2026-06-06 H2HTrack LiveGapSec TrackSec alignment landed:
  ...
  - H2HRace, Opponents, CarSA gap math/slot selection, checkpoint/gate logic, `Gap.RelativeSec`, `Car.Ahead01P/Behind01P`, export names, dashboard JSON, CompetingDrivers removal, Pace Car/self-target filters, and Property Snapshot grouping remain unchanged. Property Snapshot list reviewed: yes; no export/property add, remove, rename, or regroup.
...
- 2026-06-05 Car Tracking Probe CSV debug system landed:
  ...
  - runtime tracking behavior is unchanged: no CarSA gap math changes, no Opponents ordering changes, no H2H selector changes, no dashboard exports/properties, no Property Snapshot changes, and no `DriverInfo.CompetingDrivers` normal-driver fallback;
