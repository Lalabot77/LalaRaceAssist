- 2026-06-08 Pit Debrief box delta/fuel-target follow-up landed:
  - `Pit.Box.LastDeltaSec` now uses Pit Debrief sign convention directly (`actual elapsed - predicted target`; positive slower, negative faster), and Pit Debrief refreshes from that existing seam after countdown finalization using a separate completed-delta validity flag so valid completed-box deltas, including exact `0.0s` on-target stops, no longer remain permanently pending because of an edge-ordering tick;
  - `Pit.Debrief.Service.FuelTargetLitres` now latches the largest positive existing requested-add evidence from `Fuel.Pit.Box.WillAddLatched` / `Fuel.Pit.WillAdd` and no longer lets later zero/reset ticks overwrite a non-zero target; actual added fuel remains separate;
