...
- PitExit active-cycle prediction preserves its latched total-loss seed when shared loss estimates decrease during an active stop unless the deliberate pit-service model key changes; `PitExit.RemainingCountdownSec` already subtracts elapsed pit-cycle time from that seed, so unchanged-key decreases from natural repair countdown ageing are ignored, while higher estimates and distinguishable service-input changes can refresh the active countdown/predicted position. No dashboard property names were added/removed/renamed; PR #827-style `Fuel.Live.PitServiceRefuelRate_Lps` / `Fuel.Live.PitServiceRefuelTime_S` exports are not present. Property Snapshot list reviewed: yes; existing `Fuel.Live.*` remains Fuel/Strategy and `PitExit.*` remains Pit/PitExit, with no group changes required.
