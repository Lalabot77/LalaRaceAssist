...
- `Pit.Box.TargetSec` remains Option A service-time authority: selected/active fuel, tyres, mandatory repairs, and optional repairs when selected/active/telemetry-reported as part of service are included. Repair-influenced box targets label the debrief box text as `BOX MAND REPAIR`, `BOX OPT REPAIR`, or `BOX REPAIRS`; optional repair abandonment can produce a large negative debrief delta. No target construction, export names, settings, dashboard JSON/layout, PitCycleLite, fuel model, PitExit prediction, or `Pit.Box.LastDeltaSec` sign changed. Property Snapshot list reviewed: yes; no group mapping change required.
