- 2026-05-27: PreRace strategy-status phase cleanup (next-stint advisory split) validated.
  - Main `LalaLaunch.PreRace.StatusText` for required one-stop now keeps strategy/start-fuel validity focus during pre-grid/gridding: under-fuelled and next-stint-check branches publish `SINGLE STOP POSSIBLE` (orange) instead of next-stint refuel advisory strings.
