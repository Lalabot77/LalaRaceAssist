- 2026-06-17 Pit Entry post-line handover evidence preservation landed:
  - The immediate post-line Pit Entry handover reset now turns off active cue outputs without clearing latched entry-line evidence before Pit Debrief consumes it later in the tick, preserving `ENTRY BAD (+x.xkph / y.ym late)` detail and `Pit.Debrief.Entry.QualityText=BAD` when `Pit.Box.DistanceM < 200m` or pit-box distance authority reports `0` immediately after the line.
  - No export names, BrakeCue math, pit entry line classification, 1.0 kph line-debrief tolerance, Pit Debrief SummaryText logic, PitCycleLite, Fuel Model, PitExit prediction, dashboard JSON/layout, or Property Snapshot grouping changed. Property Snapshot list reviewed: yes; no group change required.
- 2026-06-17 Pit Entry BrakeCue text/state refinement and post-line hold landed:
  - `Pit.EntryBrakeCueText` is now the primary driver-facing BrakeCue text with exact vocabulary `OFF`, `FAULT`, `READY`, `BRAKE IN Xm`, `BRAKE NOW`, `BRAKE HARD`, `SPEED OKAY`, `BELOW LIMIT`, and `TOO SLOW`; it no longer emits `BRAKE SOON` or `LATE`. Legacy `Pit.EntryCue` / `Pit.EntryCueText` remain unchanged for compatibility.
  - Added `Pit.EntryBrakeCueState` (`0=OFF`, `1=FAULT`, `2=READY`, `3=BRAKE IN`, `4=BRAKE NOW`, `5=BRAKE HARD`, `6=SPEED OKAY`, `7=BELOW LIMIT`, `8=TOO SLOW`) for dashboard colours/animations. `BRAKE IN` counts down to the configured buffer/fake brake line via `max(0, Pit.EntryMargin_m - Pit.EntryBuffer_m)` and starts no later than 300m out; speed bands use simple ±1 kph hysteresis.
