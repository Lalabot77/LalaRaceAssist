...
- PitExit active-cycle prediction preserves its latched total-loss seed when shared loss estimates decrease during an active stop, because `PitExit.RemainingCountdownSec` already subtracts elapsed pit-cycle time from that seed; only higher shared estimates can extend the active countdown/predicted position. No dashboard property names were added/removed/renamed; PR #827-style `Fuel.Live.PitServiceRefuelRate_Lps` / `Fuel.Live.PitServiceRefuelTime_S` exports are not present. Property Snapshot list reviewed: yes; existing `Fuel.Live.*` remains Fuel/Strategy and `PitExit.*` remains Pit/PitExit, with no group changes required.
