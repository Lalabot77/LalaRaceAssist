- 2026-06-08 Pit Debrief Daytona box/fuel-target diagnostics landed:
  ...
  - bounded fix: `Pit.Debrief.Service.FuelTargetLitres` preserves positive requested-add evidence through normal refuel completion/reset when target or actual-added evidence exists, while true no-target/no-add deselects can still clear to `0.0`. `Pit.Box.LastDeltaSec` remains `target - actual` (positive quicker/better). No exports/settings/dashboard JSON/layout/PitCycleLite/fuel model/PitExit prediction paths changed. Property Snapshot list reviewed: yes; no group mapping change required.
