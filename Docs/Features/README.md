# Feature Documentation

Feature docs are user-facing workflow pages. They sit below product surfaces and core systems, and above technical subsystem docs.

- [Dashboards](Dashboards.md)
- [Strategy System](Strategy_System.md)
- [Pit Assist](Pit_Assist.md)
- [Fuel Guidance](Fuel_Guidance.md)
- [Profiles System](Profiles_System.md)
- [Shift Assist](Shift_Assist.md)
- [Race Starts](Race_Starts.md)
- [Rejoin Assist](Rejoin_Assist.md)
- [H2H System](H2H_System.md)

For architecture and ownership, see [Core systems](../Systems/README.md) and [Subsystems](../Subsystems/README.md).
