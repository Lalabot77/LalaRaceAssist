...
![Race Starts Analysis example](../Images/LaunchAnalysis.png)

For a full post-install SimHub walkthrough of the Race Starts plugin tabs and dash screen, see: [Race Starts Tutorial](https://youtu.be/_IV0simCjYo)

Think of it as two related parts:

- **live launch behavior** that supports the start itself,
- **Race Starts** for reviewing saved launch results afterwards.
...
[Race Starts Tutorial](https://youtu.be/_IV0simCjYo)
...
**Race Starts settings (currently shown in the plugin UI with launch wording where contract names remain unchanged)**

That is where you should review and adjust launch setup, including targets, tolerances, and capture behavior.

## What Race Starts Analysis is for

**Race Starts** is the separate review tab for saved launch traces and summaries.
...
- **Race Starts** helps you review what actually happened.

During the start, the system is concerned with live launch behavior. After the start, Race Starts helps you inspect saved summaries and traces so you can decide whether any tuning change is justified.
...
1. **Set a baseline** in **Race Starts settings (currently shown in the plugin UI with launch wording where contract names remain unchanged)**.
2. **Record launches** instead of changing values after every run.
3. **Review traces and summaries** in **Race Starts**.
