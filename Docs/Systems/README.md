# Core Systems

Core systems describe Lala Race Assist capabilities in product terms before the docs dive into implementation details.

- [Strategy](Strategy.md)
- [Profiles](Profiles.md)
- [Pit System](Pit_System.md)
- [Traffic Awareness](Traffic_Awareness.md)
- [Race Awareness](Race_Awareness.md)
- [Shift Assist](Shift_Assist.md)
- [Standing Start Assist](Standing_Start_Assist.md)
- [Rejoin Assist](Rejoin_Assist.md)
- [Dashboard Management](Dashboard_Management.md)
- [Monitor System](Monitor_System.md)
- [Driver Tagging](Driver_Tagging.md)
- [Developer Tools](Developer_Tools.md)

Technical ownership and contract details live in [Subsystems](../Subsystems/README.md).
