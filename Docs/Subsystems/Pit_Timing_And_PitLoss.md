...
- `Pit.LimiterSpeedKph` (plugin-owned normalized pit limiter speed in true kph for dashboard formulas). Source authority is native session `DataCorePlugin.GameRawData.SessionData.WeekendInfo.TrackPitSpeedLimit`, parsed from strings such as `80.00 kph`; it intentionally does not use `DataCorePlugin.GameData.PitLimiterSpeed` because that value can follow SimHub local/display speed units. It publishes `0` when native authority is missing/invalid and is explicitly cleared before no-game/invalid-data early returns.
...
- `Pit.Box.RunwayRangeM` / `Pit.Box.RunwayScale01` (plugin-owned Box Entry runway visual helpers). `RunwayRangeM` is `clamp(Pit.LimiterSpeedKph * 0.75, 40.0, 80.0)` metres (60 kph -> 45 m, 80 kph -> 60 m, 100 kph -> 75 m, 120 kph -> 80 m). `RunwayScale01` is `clamp(Pit.Box.DistanceM / Pit.Box.RunwayRangeM, 0, 1)` using the same wrapped forward pit-box distance, so dashboards can bind directly without duplicating NCalc range math. Both publish `0` when pit-box visibility/authority or native pit-limit authority is unavailable. These are presentation helpers only and do not change pit-box distance/time, service countdowns, Pit Debrief, PitExit prediction, or Pit Entry braking math.
- `Pit.Box.BrakeNow` (plugin-owned BoxEntry “BRAKE NOW” visibility helper). Publishes `true` only when pit-box authority + pit-limit authority are valid, speed is sane (`>2 kph`), distance is positive, pit box is within dynamic threshold `(25.0 * (pitLimitKph / 80.0)) + BoxStopCueBufferM`, and the same in-lane/pre-entry visibility gate is active. `BoxStopCueBufferM` is derived locally from the existing Pit Entry Buffer as `clamp(PitEntryBuffer_m * 0.20, 1.0, 5.0)` after the same 0–50 m Pit Entry Buffer sanitization; it biases only the Box Entry STOP/BRAKE NOW cue slightly earlier and does not change `Pit.Box.DistanceM`, `Pit.Box.TimeS`, box authority, box countdown/debrief, service timing, or pit-entry math. This legacy cue still uses the existing limiter authority chain (`DataCorePlugin.GameData.PitLimiterSpeed` primary, parsed native session fallback) to avoid changing its trigger behavior in this follow-up; publishes `false` when unavailable/invalid.
- `Pit.Box.Active` / `Pit.Box.ElapsedSec` / `Pit.Box.RemainingSec` / `Pit.Box.TargetSec` / `Pit.Box.LastDeltaSec` (driver-facing in-box service countdown contract). `Active` is true only while the player is in a valid in-box service state (`in pit lane && in pit stall && PitPhase.InBox`). `ElapsedSec` reuses the existing pit stop elapsed timer from `PitEngine` (no second timer). During the short settle period (1.0s elapsed), the live effective target is `max(modeledTargetSec, repairRemainingSec)` where `modeledTargetSec` is the shared regulation-aware fuel/tyre service result plus the existing `+1.0s` boxed-service overhead, and `repairRemainingSec` is boxed native repair-left authority (`PitRepairLeft`, plus `PitOptRepairLeft` when optional repairs are enabled). Under the Option A contract, `Pit.Box.TargetSec` means expected stop time if all currently selected/active services complete under the selected Strategy pit-service regulation: fuel, tyres, mandatory repairs, and optional repairs when selected/active/telemetry-reported as part of service. The `+1.0s` overhead is stationary box-service allowance only (boxing/settle/pickup-release slop), not lane travel. At settle threshold, that effective target is latched/frozen for the stop and exported as `Pit.Box.TargetSec`, so late stop telemetry drift no longer moves the countdown target. `RemainingSec` stays repair-aware and is computed each tick as `max(max(0, TargetSec - ElapsedSec), repairRemainingSec)`. `Pit.Box.LastDeltaSec` is computed on stop end as `(latched TargetSec - final ElapsedSec)` where positive means quicker/better than target and negative means slower/worse, preserving the existing dashboard-facing pit popup contract; the completed elapsed used for that calculation is latched alongside the delta so Pit Debrief can still resolve repair-adjusted Box Delta after the live countdown state resets. Pit Debrief repair adjustment reads mandatory plus optional repair-left telemetry through a debrief-only evidence path independent of the runtime optional-repair countdown display setting, tracks mandatory and optional repair streams separately for served-repair calculation, and derives the SummaryText repair label only from served repair evidence (`REPAIRS` when mandatory and optional both served, `MAND REPAIR` when only mandatory served, `OPT REPAIR` when only optional served) so positive but static optional repair-left telemetry is not enough to label optional repair, while `Pit.Box.TargetSec`, `Pit.Box.RemainingSec`, and `Pit.Box.LastDeltaSec` keep the runtime countdown/display semantics above. The completed delta remains latched after stop for review, clears when the next boxed stop becomes active, and clears on session/reset paths. All exports publish `0`/`false` when inactive or unavailable (including drive-throughs and missed-box states).
...
For driver-facing review, `SummaryText` now updates progressively as existing lifecycle evidence arrives: entry after pit entry, service from actual fuel-added evidence and the same current-stop tyre count used by the boxed-target resolver once a confirmed boxed-service state exists (`PlayerCarInPitStall` and pit-road state with `PitPhase.InBox`, or the existing active `Pit.Box` countdown authority), box delta from a debrief-only completed-box basis (`repair-adjusted driver-controlled elapsed - repair-excluded modeled service target`), and `STRAT Δ` after final out-lap timing. A raw or stale pit-stall signal while the player is not on pit road, while PitEngine is not in the boxed-service phase, or while PitEngine is already `ExitingPits` is not a Pit Debrief box-entry edge and does not call `LatchBoxEntry`. `Pit.Debrief.Entry.SummaryText` is the plugin-owned entry-only first section of full `Pit.Debrief.SummaryText`; the full summary and entry-only export share the same entry-section formatter. The entry headline is performance-oriented from the latched raw-double entry line time loss (formatted to one decimal) for `safe`/`normal` line-compliance tokens, while the Pit Entry Assist safety/compliance token remains available in limiter/debug/log evidence. Marginal pit-entry line overspeed within +1.0 kph is intentionally kept as the existing `normal` compliance token, so `Pit.Debrief.Entry.LimiterQualityText` remains `NORMAL` and the timing headline remains eligible. True overspeed above +1.0 kph keeps `Pit.Debrief.Entry.LimiterQualityText=POOR`; when Pit Debrief has current-stop line evidence plus speed above +1.0 kph or meaningful late distance, it publishes `Pit.Debrief.Entry.QualityText=BAD` and overrides the compact entry section to `ENTRY BAD` with structured compliance detail (`+x.xkph` and late metres when available), not the timing-loss bracket. Stale previous-stop bad tokens without current-stop evidence are ignored for the override. Box delta means repair-adjusted driver-controlled boxed/service duration minus the repair-excluded predicted fuel/tyre/boxed-overhead service duration (positive slower, negative faster); the countdown exit latch falls back to the last live elapsed value if PitEngine stop duration is not ready on the first box-exit edge, and Pit Debrief refreshes from the completed countdown validity flag plus its own frozen repair-excluded target so exact `0.0s` deltas are valid without reusing a previous stop's delta. `Pit.Box.TargetSec`, `Pit.Box.RemainingSec`, and `Pit.Box.LastDeltaSec` remain repair-aware for the runtime countdown/pit-popup contract, but Pit Debrief Box Delta excludes both optional and mandatory repair duration from the target side and removes served repair contribution from the elapsed side when repair-left evidence is available, so optional repair abandonment does not create a misleading large negative driver-performance delta and waiting for repairs does not create a misleading positive driver-performance delta. `BOX ... (Δ PENDING)` means completed elapsed/target evidence was unavailable, invalid, or not completed yet; preliminary box-entry target/elapsed evidence is not treated as a completed box delta. Service/repair context remains visible: when repair time influenced the boxed target, the driver-facing `SummaryText` box label becomes `BOX MAND REPAIR`, `BOX OPT REPAIR`, or generic `BOX REPAIRS`; `Pit.Debrief.Box.QualityText` remains the quality enum (`GOOD` for valid non-missed boxed stops, `OVERSHOT`/`MISSED` for missed-box phases, `UNKNOWN` when unavailable). Verbose `[LalaPlugin:PitDebriefBoxDiag]` diagnostics trace the existing countdown/debrief latch path only when master debug and `Enable Debug Logging` are both enabled; they do not change PitEngine phase or timing ownership. The final `[LalaPlugin:PitDebrief]` summary log remains always-on. Exit prediction remains exported and logged under `Pit.Debrief.Exit.*`, but it is no longer included in `SummaryText`.
