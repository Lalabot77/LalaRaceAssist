...
For driver-facing review, `SummaryText` now updates progressively as existing lifecycle evidence arrives: entry after pit entry, service from actual fuel-added evidence and tyre count, box delta from the existing `Pit.Box.TargetSec`/`Pit.Box.LastDeltaSec` countdown seam, and `STRAT Δ` after final out-lap timing. Exit prediction remains exported and logged under `Pit.Debrief.Exit.*`, but it is no longer included in `SummaryText`.
