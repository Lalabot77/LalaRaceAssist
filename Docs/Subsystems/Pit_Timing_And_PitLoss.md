...
For driver-facing review, `SummaryText` now updates progressively as existing lifecycle evidence arrives: entry after pit entry, service from actual fuel-added evidence and tyre count, box delta from the existing `Pit.Box.TargetSec`/`Pit.Box.LastDeltaSec` countdown seam, and `STRAT Δ` after final out-lap timing. The entry headline is performance-oriented from the latched entry line time loss, while the original Pit Entry Assist safety/compliance token remains available in limiter/debug/log evidence. Box delta means actual boxed/service duration minus predicted boxed/service duration; the countdown exit latch falls back to the last live elapsed value if PitEngine stop duration is not ready on the first box-exit edge, and Pit Debrief suppresses implausible deltas (`abs(delta) > 10.0s`) to `Δ PENDING` rather than showing impossible numbers. Exit prediction remains exported and logged under `Pit.Debrief.Exit.*`, but it is no longer included in `SummaryText`.
