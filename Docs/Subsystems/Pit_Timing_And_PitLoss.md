...
- Boxed-stop runtime predictions add stopped-box components separately:
  - boxed service model (`max(fuelTime, tireTime) + 1.0s` stationary service overhead, repair-aware), where runtime `tireTime` is now selected-tyre-count aware (`Fuel.Live.TireChangeCount` + scaled `Fuel.Live.TireChangeTime_S`, fail-open to 4 tyres if tyre flags are unavailable),
