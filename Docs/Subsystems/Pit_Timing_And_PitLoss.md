...
- Boxed-stop runtime predictions add stopped-box components separately:
  - boxed service model (`max(fuelTime, tireTime) + 1.0s` stationary service overhead, repair-aware), where runtime `tireTime` is now selected-tyre-count aware (`Fuel.Live.TireChangeCount` + scaled `Fuel.Live.TireChangeTime_S`, fail-open to 4 tyres if tyre flags are unavailable or partial, strategy-slider independent (profile full-4 base)), boxed-target latch freezes pre-service selected tyre count at in-box activation so in-service flag clear-down cannot reduce active `Pit.Box.TargetSec`,
...
For driver-facing review, `SummaryText` now updates progressively as existing lifecycle evidence arrives: entry after pit entry, service from actual fuel-added evidence and tyre count, box delta from the existing `Pit.Box.LastDeltaSec` completed-box seam (inverted internally for Pit Debrief sign), and `STRAT Δ` after final out-lap timing. The entry headline is performance-oriented from the latched entry line time loss, while the original Pit Entry Assist safety/compliance token remains available in limiter/debug/log evidence. Box delta means actual boxed/service duration minus predicted boxed/service duration (positive slower, negative faster); the countdown exit latch falls back to the last live elapsed value if PitEngine stop duration is not ready on the first box-exit edge, and Pit Debrief also refreshes from the completed `Pit.Box.LastDeltaSec` seam after countdown finalization using a per-stop validity flag so exact `0.0s` deltas are valid without reusing a previous stop's delta. Pit Debrief suppresses implausible deltas (`abs(delta) > 10.0s`) to `Δ PENDING` rather than showing impossible numbers. Edge-only `[LalaPlugin:PitDebriefBoxDiag]` diagnostics trace the existing countdown/debrief latch path when a boxed stop still needs review; they do not change PitEngine phase or timing ownership. Exit prediction remains exported and logged under `Pit.Debrief.Exit.*`, but it is no longer included in `SummaryText`.

`Pit.Debrief.Service.FuelTargetLitres` is still a debug/readout target sourced only from existing `Fuel.Pit.Box.WillAddLatched` / `Fuel.Pit.WillAdd` evidence. Pit Debrief preserves positive requested-add evidence when `_isRefuelSelected` drops false after natural refuel completion/reset, but a selected→deselected edge while boxed before natural completion is treated as an explicit cancel and clears the current debrief target to `0.0`; true no-refuel/pre-flow cancel stops also clear stale target evidence. `FuelAddedLitres` remains separate actual-added evidence and is not used as a fake target. Edge-only `[LalaPlugin:PitDebriefFuelDiag]` logs report the source evidence, explicit-cancel latch, natural-completion result, and clear/preserve action.
