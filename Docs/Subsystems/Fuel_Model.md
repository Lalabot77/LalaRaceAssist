...
- live tyre-service estimate (`Fuel.Live.TireChangeTime_S`) and runtime selected tyre count (`Fuel.Live.TireChangeCount`) from per-wheel DP tyre flags (fail-open to 4 tyres when flags are unavailable).
