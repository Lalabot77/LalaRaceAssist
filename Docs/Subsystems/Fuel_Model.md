...
- `LalaLaunch.PreRace.FuelDelta`: single pre-race delta output. `Auto` remains planner-first for totals/stints, but delta is now live against the actual pit-menu refuel request (`PitSvFuel`) versus planner-required next add (`PlannerNextAddLitres`) when planner data exists. This means Auto no longer reports zero just because planner data is present; it reports the current add shortfall/excess until the pit menu matches planner intent. If next-add data is unavailable, Auto uses the existing planner fuel-basis fallback. `Single Stop` uses current fuel + pit-menu refuel intent; `No Stop`/`Multi Stop` use current fuel only.
