...
- Non-player `Drivers##` rows flagged as Pace Car are excluded before native race-order rows are built. The conservative pace-car helper checks `IsPaceCar`, `CarIsPaceCar`, exact `UserName`/`AbbrevName`/`CarScreenName` label `Pace Car`, and `CarPath` values containing `pacecar`; it never filters solely by `CarIdx`, so `PlayerCarIdx == 0` remains valid. If the current player row is pace-car flagged, Opponents preserves the player row and emits a one-time warning instead of blindly removing the player.
