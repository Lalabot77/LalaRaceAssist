...
- Phase split:
  ...
  - **Active pit-cycle phase** (`onPitRoad || pitTripActive`): latches player active-cycle start and uses a simple remaining pit-cycle countdown (`remaining = latched total stop loss - elapsed session time in cycle`, floored at 0) rather than repeatedly treating the stop as a fresh full-loss event. While the cycle is active, the latched total only refreshes upward when the shared stop-loss estimate increases; lower live estimates are ignored because elapsed cycle time is already applied in the countdown.
