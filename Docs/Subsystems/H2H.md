...
- When a lap wraps, the just-finished closing segment is carried across the runtime reset so sector 6 can still publish valid timing after start-finish; previously published valid segment states/deltas stay latched on the dash until the new lap rebuilds that same segment, and the new lap progressively overwrites each segment once replacement timing is available.
...
- When a lap wraps, the outgoing active segment is finalized and its completion time is carried across the runtime reset so all six fixed segments, including sector 6, remain publishable; already-published segment outputs then stay visible until the new lap revisits and replaces them.
