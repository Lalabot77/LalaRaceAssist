...
- `LastLapColor` uses direct dash-ready `#RRGGBB` outputs: white `#FFFFFF` for a normal last lap, lime `#00FF00` when the corresponding published last lap is also that participant's valid PB, and magenta `#FF00FF` when that valid PB also matches the same-class session-best; session-best overrides PB, and invalid faster raw last laps do not trigger PB/session-best coloring. Class session-best resolution is native-only; in defensibly single-class sessions (`WeekendInfo.NumCarClasses == 1`, with native multi-opponent hint as supporting context), blank class strings no longer block same-class resolution. In multiclass sessions, blank class identity still fails safe and class-best remains unresolved (`0`) rather than collapsing classes.
