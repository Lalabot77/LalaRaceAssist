...
- `FUEL LEARNING STALE` observes accepted fuel-learning sample progression only. It uses existing accepted-sample counters/windows and waits until at least lap 5, then requires roughly 3 completed laps without accepted fuel-learning progress before warning; existing accepted-sample counter resets/decreases rebaseline the monitor and clear stale observation. It does not decide whether any particular lap should have been accepted.
...
Inputs are limited to SimHub/iRacing telemetry plus the existing pre-race setup-fuel seam:
- current fuel: `DataCorePlugin.GameData.NewData.Fuel` (`GameData.NewData.Fuel` in the plugin data object);
- fuel per lap: `DataCorePlugin.Computed.Fuel_LitersPerLap`;
- remaining laps: `DataCorePlugin.GameData.LapsRemaining`;
- MFD fuel add: `DataCorePlugin.GameRawData.Telemetry.PitSvFuel`;
- refuel enabled: `DataCorePlugin.GameRawData.Telemetry.dpFuelFill`;
- pre-race setup fuel source: existing `Fuel.Setup.FuelLevel` / `Fuel_Setup_FuelLevel` seam populated from car setup fuel-level candidates.

Formula:
- pre-race/grid available fuel = valid setup fuel, otherwise current fuel;
- predictive / pit-road-entry / pit-box-entry available fuel = current fuel + `PitSvFuel` only when `dpFuelFill` is enabled;
- pit-road-exit available fuel = current fuel only, because the stop is complete and a still-selected MFD request is not available for the just-completed stop;
...
The check fails silently when current fuel is invalid or zero/non-positive, or when fuel-per-lap or remaining laps are invalid/non-positive. It adds no telemetry-validity layer, fallback substitution, planner/strategy consumption, contingency/formation/after-zero logic, `Fuel.Refuel.*` change, Pit Fuel Control change, Pit Command change, export, health-logic-specific CSV, or per-tick evaluation. It only runs on existing MonitorSystem edges: predictive two-laps-remaining, pit-road entry, pit-box entry, and pit-road exit; the pit-road-exit edge intentionally excludes selected MFD fuel and evaluates actual exit fuel only. When pit-road exit fires, the same update skips the predictive block so a pit-exit WARNING is not downgraded to a predictive CAUTION before the tick ends. Once finish authority reaches `Race.EndPhase >= 2`, only predictive/off-pit-road CAUTION `BASELINE SHORT` is suppressed as late-race guidance noise; pit-road, pit-box, and pit-exit WARNING `BASELINE SHORT` contexts remain eligible and no fuel/baseline math changes.
...
- The selector uses the validated `Fuel.Live.RemainingStints` basis only after runtime refuel/projection context has been resolved; reset/default `0` without the internal validity bit is not treated as END.
- Phase selection uses `effectiveTargetStints = Fuel.Live.RemainingStints + 1.0` because `Fuel.Live.RemainingStints` excludes the current stint. Selection order is `STINT` (`effectiveTargetStints > 2.0`, equivalent raw `Fuel.Live.RemainingStints > 1.0`, target = `Fuel.StintBurnTarget`), `SESSION` (`effectiveTargetStints > 1.0`, equivalent raw `Fuel.Live.RemainingStints > 0.0 && <= 1.0`, target = session calculation), `END` (`effectiveTargetStints <= 1.0`, equivalent raw `Fuel.Live.RemainingStints == 0.0`, target = `Fuel.RequiredBurnToEnd`, subject to validity and END reserve guard), else `INVALID`/`0`/`false`.
- Only SESSION requires raw MFD request availability; STINT and END ignore that seam. SESSION uses the raw driver-selected MFD fuel request seam (`PitSvFuel`, gated to zero when `dpFuelFill` says refuel is not selected) before tank-space clamp, then applies a fixed stationary no-burn pit-box service-time credit: `(min(current fuel + selected MFD request, max tank) - active contingency litres + stationaryNoBurnCreditLitres) / Fuel.LiveLapsRemainingInRace_Stable`, where `stationaryNoBurnCreditLitres = ceil(validated Fuel.Live.RemainingStints) * 40.0s * (Fuel.Refuel.SelectedBurnPerLap / selected runtime refuel projection lap seconds)`. This credit accounts for the stationary car not burning fuel during pit-box service time; it is not a timed-race distance or required-laps reduction, so timed projections and lap-limited fallback projections keep the same `Fuel.LiveLapsRemainingInRace_Stable` remaining-lap denominator while still receiving the stationary no-burn box-time credit when selected burn, projection lap seconds, and remaining stops are valid/positive. If selected burn, projection lap seconds, or remaining stops are invalid/non-positive, SESSION falls back to the prior no-credit calculation instead of invalidating. It does not use `Fuel.Refuel.NextLitresCeil`, `Fuel.Pit.WillAdd`, plugin-recommended refuel values, planner fuel values, or tank-space-clamped add values to decide selected fuel or stop count.
...
- an existing unhealthy observation publishes `CHECK FUEL DATA`,
- an attempted recovery publishes `FUEL DATA RECOVERED` or `FUEL DATA FAULT` from the existing health verdict; deferred/not-attempted recovery retains `CHECK FUEL DATA`. `FUEL DATA RECOVERED` is held for 10 seconds as confirmation, then clears directly to `MONITOR READY` only if MonitorSystem is still enabled and the active text is still exactly `FUEL DATA RECOVERED`; it does not step through `FUEL HEALTH OK` and does not clear replacement warnings/faults/stale-state messages.

The monitor does not change fuel-model learning, the two-sample unhealthy streak, the 450 ms evaluation throttle or two-second recovery throttle, or runtime fuel values. Phase 2B pit-stop warnings are limited to edge-triggered cross-checks against existing Phase 2A snapshots/refuel seams; Phase 2C adds only the separate edge-triggered `BASELINE SHORT` SimHub gross sanity check described above. Fuel-model gross-check warnings remain out of scope. A transient `CHECK FUEL DATA` clears to `FUEL HEALTH OK` when a later evaluation satisfies the existing valid healthy pass basis. Recovery rejected by the existing throttle (or otherwise not attempted) remains WATCH rather than being classified as FAULT; `FUEL DATA FAULT` is reserved for an attempted recovery whose existing post-refresh health verdict is false. Manual targeted recovery publishes the same outcome mapping while retaining its existing successful early-return and unsuccessful/deferred broad-reset behavior.

Phase 2A adds pit-stop trigger evidence only. It observes existing Fuel Model/Pit Fuel Control/Pit Timing state to log edge-only trigger moments and freezes a lightweight pit-entry snapshot for later checks. It does not change `Fuel.Refuel.*`, `Fuel.Pit.*`, fuel-health recovery, Pit Fuel Control DATA/MODE/SOURCE behavior, or pit command sending.

Phase 2B adds edge-triggered `MonitorSystem.*` pit-stop warnings using only the Phase 2A snapshot/evidence fields and existing plugin refuel recommendation seams. The warning basis is the existing runtime recommendation `Fuel.Refuel.NextLitres` / `Fuel.Refuel.Valid`, not the MFD-selected add amount (`PitSvFuel`), MFD refuel enable state (`dpFuelFill`), or `Pit_WillAdd`/`Fuel.Pit.WillAdd` mirror of the currently selected add. This keeps `REFUEL OFF` valid when refuel is disabled and keeps `MFD FUEL LOW` comparing the selected MFD add against an independent recommendation. MonitorSystem trusts the SimHub/iRacing telemetry seams used for these simple race-risk checks (`dpFuelFill` and `PitSvFuel`) and does not add telemetry-validity/fallback layers around hypothetical missing SimHub values; missing SimHub telemetry is outside MonitorSystem scope unless real project logs prove a problem. If `Fuel.Refuel.Valid` is false or the recommendation is not finite/non-negative, Phase 2B service warnings fail closed. Predictive two-laps-fuel checks can publish `REFUEL OFF` or `MFD FUEL LOW` as CAUTION; pit-road entry, pit-box entry, and on-pit-road/boxed Fuel Control mode changes can publish those messages as WARNING; pit-road exit can publish `EXIT FUEL SHORT` as WARNING when the pit-entry snapshot had a meaningful required add (`PluginNextLitres > 0.5L`) and actual exit fuel is below the pit-entry snapshot's expected fuel-on-exit. A first tick that starts while already on pit road seeds the pit-entry snapshot for later exit checking but does not publish a warning on that priming tick. A clean relevant edge, including an off-pit-road Fuel Control mode edge while still inside the predictive low-fuel window after an earlier clean one-shot predictive pass, clears an active Phase 2B pit-warning text back to `MONITOR READY`. Unresolved fuel-health alerts (`CHECK FUEL DATA` or `FUEL DATA FAULT`) block pit-warning publication/clearing; auto-recoverable `FUEL DATA RECOVERED` does not block urgent pit warnings. The refuel-off check is guarded by refuel completion (`CurrentFuel + 0.75L >= pit-entry PluginFuelOnExit` or `PluginNextLitres <= 0.5L`) so the normal late switch to OFF after fuel has been added is not warned. After the existing pit-road-exit seam fires, MonitorSystem also inhibits only `REFUEL OFF` until the first completed outlap after exit is complete by latching `CompletedLaps` at exit and suppressing while current completed laps is less than or equal to that latched value; `MFD FUEL LOW`, `EXIT FUEL SHORT`, `BASELINE SHORT`, fuel-health faults, stale-state, and Car/Opp/H2H warnings remain eligible. Suppressed `REFUEL OFF` does not publish a MonitorSystem event or Monitor Event CSV row and only emits concise log evidence. Phase 2C adds `BASELINE SHORT` as the separate SimHub-only baseline sanity check described above. Predictive, pit-road-entry, and pit-box-entry baseline checks can include selected MFD fuel when refuel is enabled; pit-road-exit baseline checks use actual exit fuel only. MonitorSystem still does not add `FUEL MODEL CHECK`, planner/after-zero/remaining-stint/contingency recomputation, pit command behavior, automatic command sending, new exports, or fuel-health-specific CSV spam. The optional debug-only Monitor Event CSV records changed non-normal MonitorSystem publications only when explicitly enabled.
...
- Reason: `MonitorSystem.*` is already registered through `AttachCore` and mapped by the existing `MonitorSystem.` prefix to the Fuel/Strategy Property Snapshot group; no new export/property or snapshot group was added.
