...
- Pre-green adapter contract: `LalaLaunch.PreRace.*` and `StrategyDash.*` include Formation Lap Fuel exactly once in pre-green total-need guidance (`race requirement + contingency + formation`), while runtime `Fuel.*` tactical exports remain formation-excluded.
- Pit add requirement.
