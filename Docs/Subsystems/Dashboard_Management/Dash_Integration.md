...
`LalaRaceAssist.VersionManifest.json` at the repository root is the release/version source of truth for v1.1 plugin and dashboard package audits. The same file is embedded into `LaunchPlugin.dll` at build time for runtime Overview/export use, so normal installs do not require users or install scripts to copy the JSON beside the DLL. The manifest records the current plugin release identity, the compatible plugin family, one entry per distributable dashboard/overlay asset, and release-audit metadata for the official SimHub `.simhubdash` export packages.

Repository release validation and runtime installed detection intentionally use different sources. The repository stores official `Dash Files/*.simhubdash` export packages as the release artefacts; unpacked dashboard folders are no longer required in the repository. The release audit opens those export packages directly and reads only the declared `releaseAudit.rootFile` `.djson` from each archive. Runtime installed detection remains separate: it reads the user's installed SimHub `DashTemplates` folders through the existing manifest `folder` / `file` / `versionProperty` fields. Do not repurpose those runtime fields for release package paths.

Dashboard and overlay package assets are versioned individually through their main/root `.djson` `DashboardVersion` metadata inside the official export package. Shared copied widgets are **not** independently versioned: PitEntry, BoxEntry, PitPopup, RejoinAssist, LaunchAssist, copied H2H/Fuel/traffic/alert widgets, and similar common surfaces inherit the version of the containing dashboard or overlay package. The audit must not recursively accept arbitrary copied-widget `DashboardVersion` fields; only the manifest-declared release root file determines the package version.
...
The plugin consumes the embedded copy of the root manifest at runtime for bundled dashboard/overlay compatibility display and export. Runtime manifest validation requires the official six-asset release catalogue above to be complete and valid, including parseable `v?major.minor[.patch[.revision]]` latest versions, before `Dashboards.Manifest.Valid` becomes true. That completeness check validates the release catalogue only; it does **not** mean a user has installed all six assets, and users may install only the dashboards/overlays they need.

The Overview page now separates the bundled compatibility contract from local installation state. Bundled versions come from the embedded manifest and remain exposed through `Dashboards.<Package>.ExpectedVersion`; `LatestVersion` is still the bundled manifest value until future GitHub/latest manifest work lands. Installed versions are detected locally by treating the loaded plugin DLL directory as the primary SimHub root, validating its `DashTemplates` folder, falling back to the parent `DashTemplates` folder only for non-standard layouts, and reading only the deterministic manifest-declared root `.djson` path for each package. The scanner maps manifest package folders such as `Dash Files/Lala-Driver Dash` to installed SimHub folders such as `DashTemplates/Lala-Driver Dash`, opens the manifest-declared root file, and reads exact `Metadata.DashboardVersion` metadata. It does not recursively search `DashTemplates`, does not scan arbitrary folders, and does not inspect dashboard JavaScript.

Installed package status is deliberately simple in v1.1: `INSTALLED` means the expected root `.djson` was found and `Metadata.DashboardVersion` was present and parseable; `NOT INSTALLED` means the expected package folder/file was not found; `UNKNOWN` means SimHub root detection failed, the JSON could not be read, or the version metadata was missing/blank/unparseable. Overview scans installed dashboards when the Overview opens and when **Check Now** is pressed, then caches those results; it must not perform filesystem IO during the recurring status refresh timer. Dash-side RED/AMBER/GREEN warning panels, GitHub latest package checking, update-available comparison, and automatic download/install remain deferred future work.
...
The audit reports assembly version attributes, confirms the manifest is embedded in the project, reports manifest plugin version fields, manifest asset latest versions, opens each manifest-declared `.simhubdash` package without extracting it, reads the declared root `.djson` member with UTF-8-SIG tolerant decoding, and reports any missing package, non-zip package, missing/ambiguous root file, invalid JSON, missing/blank/unparsable version, or manifest/package version mismatch. Release-critical and non-critical mismatches are reported separately. The audit must not edit files; fix mismatches manually in the manifest or root dashboard metadata, export the dashboard package again from SimHub, then rerun the audit.

Release package workflow: edit the dashboard in SimHub, update the root `Metadata.DashboardVersion`, export the dashboard from SimHub to `Dash Files/*.simhubdash`, update `LalaRaceAssist.VersionManifest.json`, run `python3 Docs/Internal/VersionManifestAudit.py`, build the plugin so the manifest is embedded, and release the plugin with the exported dashboard packages.
...
Dashboard logic should use `MonitorSystem.Text` and the paired colour exports rather than reconstructing fuel-health conditions. The persisted `Enable Monitor System` setting defaults on and is shown under `Settings -> Plugin Monitor`; dashboards/hardware can toggle it with `LalaLaunch.MonitorSystemToggle`: enabled state starts as `ON` / `MONITOR READY` / enum `1`; disabled state is `OFF` / `MONITOR OFF` / enum `0`; re-enabling returns to the enabled ready state. Fuel-health, pit-stop, baseline, Car/Opp/H2H, and stale-state warning messages publish only while enabled. `FUEL DATA RECOVERED` is a short confirmation state only: after a 10 second hold, it clears directly to `MONITOR READY` if MonitorSystem is still enabled and the active text is still exactly `FUEL DATA RECOVERED`; replacement warnings/faults/stale-state messages are left untouched. CAUTION/WATCH styling is yellow background with black text; WARNING/FAULT styling is red background with yellow text. Message definitions follow `Docs/Internal/MonitorSystem_Messages.csv`; its categories intentionally match the Monitor Event CSV `Category` values.
...
The debug-only `Enable Monitor Event CSV` setting writes non-normal MonitorSystem publication changes (`WATCH`, `CAUTION`, `WARNING`, `FAULT`, `RECOVERED`) to `Logs/LalaPluginData/MonitorSystem_Events.csv`. It can also write evidence-only `Lifecycle` rows (`OPPONENTS RESTARTED`, `CARSA RESTARTED`, `H2H RESTARTED`, optional `LAPREF RESTARTED`) after an existing lifecycle/manual reset fanout when the subsystem first reaches a conservative healthy proxy; those rows bypass MonitorSystem publication and do not change `MonitorSystem.Text`, warning priority, exports, or dashboard behavior. It does not write `MONITOR OFF`, `MONITOR READY`, `FUEL HEALTH OK`, the automatic `FUEL DATA RECOVERED` -> `MONITOR READY` clear, or per-tick OK rows; file I/O failure disables the CSV writer after one warning. Shift Assist remains out of MonitorSystem unless real logs prove a monitoring need.
...
- Player snapshot trigger hierarchy is plugin-owned: per-car finish-like flags are used only after the plugin finish-lifecycle stale-flag gate, robust player-checkered seams (`GameData.Flag_Checkered` / `SessionFlagsDetails.IsCheckered*`) are used before `SessionState==6` safety fallback. Dashboards must consume exported snapshot flags and must not reimplement finish crossing heuristics.
...
- Pit service regulation math is plugin-owned. Dashboards consume existing pit/strategy exports (`Fuel.Live.RefuelRate_Lps`, `Fuel.Live.TotalStopLoss`, `Pit.Box.*`, `PitExit.*`, `StrategyDash.*`, `PreRace.*`) and must not reimplement Default/IMSA/NEC fuel-vs-tyre timing formulas. `Fuel.Live.RefuelRate_Lps` is the active effective runtime service rate: Default/IMSA mirror the stored normal profile/FuelCalculator baseline, while NEC applies the selected car-profile NEC percentage factor at export time without changing stored `CarProfile.RefuelRate`.
...
- Smooth pit-window range bars should use `Fuel.PitWindow.SelectedOpeningLap` / `Fuel.PitWindow.SelectedClosingLap` when they need decimal lap endpoints that follow the selected runtime PUSH/NORM/SAVE burn basis. These selected endpoints are independent of the existing stable/NORM state-machine window-needed gate and stable-fuel confidence readiness, so valid selected fallback recommendations can still drive the smooth range. They fail closed to `0.0` / `0.0` for impossible inverted intervals. Keep `Fuel.PitWindowOpeningLap` / `Fuel.PitWindowClosingLap` for the existing state-machine integer markers and do not rebuild plugin fuel-window math in dashboard formulas.
...
- Built-in pit command rows are user-configured in plugin **Bindings → Pit Commands**; dashboards should bind those action ids directly and not carry raw transport syntax.
- Custom chat buttons should bind to plugin-owned actions `LalaLaunch.CustomMessage01..LalaLaunch.CustomMessage10` (configured in Bindings → Custom Messages) rather than embedding transport/raw chat syntax in dash logic.
...
- In plugin Bindings → Pit Commands, tyre-control binding UI intentionally exposes only `Tyre Mode Cycle`; direct `SetOff/SetDry/SetWet/SetAuto` actions remain available for direct binding in SimHub Controls & Events / Dash Studio.
...
- `LalaDashShow*` = `DRIVER` column in Settings -> Dashboards = Lala Race Dash visibility
...
- `Pit.EntryCue`
- `Pit.EntryCueText`
- `Pit.EntryBrakeCueText`
- `Pit.EntryBrakeCueState`
...
- `RejoinCurrentPitPhase` mirrors the plugin-owned pit phase and must continue progressing during first-lap/pre-race pit-road passes where pit-loss learning is suppressed; dashboards should still expect Box Entry widgets to become eligible from `ApproachingBox` plus valid `Pit.Box.DistanceM` evidence rather than a permanently latched `EnteringPits` phase.
...
Pit Debrief box/fuel diagnostics are SimHub log-only source traces. They add no dashboard JSON/layout contract, no new exports, and no regrouping. Box target modeling uses the frozen current-stop latched tyre count, so confirmed fuel-only stops must not carry stale/default 4-tyre timing and real tyre stops are not overwritten by in-service flag clear-down; the 4-tyre fallback is retained when tyre-service evidence is unavailable before service. `Pit.Debrief.Service.FuelTargetLitres` remains debug/readout-only and preserves positive requested-add evidence through normal refuel completion/reset or deselect when positive fuel movement exists and added fuel is within the completion-tolerance window, while still clearing explicit in-box refuel-cancel before natural completion and true no-refuel/pre-flow cancel cases.

## Dash Control actions
Dash Control binding actions include `LalaLaunch.DarkModeBrightnessDecrease`, `LalaLaunch.DarkModeBrightnessIncrease`, `LalaLaunch.ShiftAssistToggle`, `LalaLaunch.StrategyContingencyAdd1L`, and `LalaLaunch.StrategyContingencyRemove1L`. The brightness actions adjust the same `Settings.DarkModeBrightnessPct` value used by the Settings -> Dashboards brightness slider in 5 percentage point steps, clamp to the slider's `0..100` range, save settings, and leave Dark Mode Off/Manual/Auto and Lovely True Dark ownership semantics unchanged. Shift Assist delegates to the subsystem enable setting and notifies the Profiles checkbox ViewModel. Strategy contingency +/- are litre controls: they switch the planner to Extra Litres mode when needed, keep the existing numeric value, then apply +/-1L through the Strategy planner property path.
