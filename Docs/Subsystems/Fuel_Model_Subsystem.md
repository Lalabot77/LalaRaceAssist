...
- race-running/pit available fuel = current fuel + `PitSvFuel` only when `dpFuelFill` is enabled;
...
The check fails silently when current fuel, fuel-per-lap, or remaining laps are invalid/non-positive. It adds no telemetry-validity layer, fallback substitution, planner/strategy consumption, contingency/formation/after-zero logic, `Fuel.Refuel.*` change, Pit Fuel Control change, Pit Command change, export, CSV, or per-tick evaluation. It only runs on existing MonitorSystem edges: predictive two-laps-remaining, pit-road entry, pit-box entry, and pit-road exit.
