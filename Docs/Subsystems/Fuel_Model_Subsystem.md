...
Phase 2B adds edge-triggered `MonitorSystem.*` pit-stop warnings using only the Phase 2A snapshot/evidence fields and existing plugin refuel recommendation seams. The warning basis is the existing runtime recommendation `Fuel.Refuel.NextLitres` / `Fuel.Refuel.Valid`, not the MFD-selected add amount (`PitSvFuel`), MFD refuel enable state (`dpFuelFill`), or `Pit_WillAdd`/`Fuel.Pit.WillAdd` mirror of the currently selected add. This keeps `REFUEL OFF` valid when refuel is disabled and keeps `MFD FUEL LOW` comparing the selected MFD add against an independent recommendation. If `Fuel.Refuel.Valid` is false or the recommendation is not finite/non-negative, Phase 2B warnings fail closed. Predictive two-laps-fuel checks can publish `REFUEL OFF` or `MFD FUEL LOW` as CAUTION; pit-road entry, pit-box entry, and on-pit-road/boxed Fuel Control mode changes can publish those messages as WARNING; pit-road exit can publish `EXIT FUEL SHORT` as WARNING when the pit-entry snapshot had a meaningful required add (`PluginNextLitres > 0.5L`) and actual exit fuel is below the pit-entry snapshot's expected fuel-on-exit. A first tick that starts while already on pit road seeds the pit-entry snapshot for later exit checking but does not publish a warning on that priming tick. A clean relevant edge, including an off-pit-road Fuel Control mode edge after a predictive warning, clears an active Phase 2B pit-warning text back to `MONITOR READY` without clearing or overwriting fuel-health monitor texts (`FUEL DATA CHECK`, `FUEL DATA FAULT`, or `FUEL DATA RECOVERED`). The refuel-off check is guarded by refuel completion (`CurrentFuel + 0.75L >= pit-entry PluginFuelOnExit` or `PluginNextLitres <= 0.5L`) so the normal late switch to OFF after fuel has been added is not warned. Phase 2B does not add `BASELINE SHORT`, `FUEL MODEL CHECK`, independent gross SimHub baseline maths, planner/after-zero/remaining-stint/contingency recomputation, pit command behavior, automatic command sending, CSV writing, or new exports.
