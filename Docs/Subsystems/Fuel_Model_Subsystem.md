...
Phase 2B adds edge-triggered `MonitorSystem.*` pit-stop warnings using only the Phase 2A snapshot/evidence fields and existing plugin refuel seams (`PluginNextLitres`, `PluginFuelOnExit`) as recommendation cross-checks. Predictive two-laps-fuel checks can publish `REFUEL OFF` or `MFD FUEL LOW` as CAUTION; pit-road entry, pit-box entry, and on-pit-road/boxed Fuel Control mode changes can publish those messages as WARNING; pit-road exit can publish `EXIT FUEL SHORT` as WARNING when actual exit fuel is below the pit-entry snapshot's expected fuel-on-exit. The refuel-off check is guarded by refuel completion (`CurrentFuel + 0.75L >= pit-entry PluginFuelOnExit` or `PluginNextLitres <= 0.5L`) so the normal late switch to OFF after fuel has been added is not warned. Phase 2B does not add `BASELINE SHORT`, `FUEL MODEL CHECK`, independent gross SimHub baseline maths, planner/after-zero/remaining-stint/contingency recomputation, pit command behavior, automatic command sending, CSV writing, or new exports.
