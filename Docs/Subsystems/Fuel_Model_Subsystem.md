...
- Only SESSION requires raw MFD request availability; STINT and END ignore that seam. SESSION uses the raw driver-selected MFD fuel request seam (`PitSvFuel`, gated to zero when `dpFuelFill` says refuel is not selected) before tank-space clamp, then applies a fixed pit-stop fuel-burn credit: `(min(current fuel + selected MFD request, max tank) - active contingency litres + pitFuelCreditLitres) / Fuel.LiveLapsRemainingInRace_Stable`, where `pitFuelCreditLitres = ceil(validated Fuel.Live.RemainingStints) * 40.0s * (Fuel.Refuel.SelectedBurnPerLap / selected runtime refuel projection lap seconds)`. If selected burn, projection lap seconds, or remaining stops are invalid/non-positive, SESSION falls back to the prior no-credit calculation instead of invalidating. It does not use `Fuel.Refuel.NextLitresCeil`, `Fuel.Pit.WillAdd`, plugin-recommended refuel values, planner fuel values, or tank-space-clamped add values to decide selected fuel or stop count.
