...
- The selector is owned by the Fuel Model runtime seam and uses `Fuel.Live.RemainingStints` as the phase signal only when the runtime refuel/stints basis is valid (`Fuel.Refuel.Valid=true`): `>2` selects STINT/current-stint management, `>1` and `<=2` selects SESSION/final-stop-capable session-to-end management, and valid `<=1` selects END/final-stint burn-to-end management.
- STINT validity requires the existing `Fuel.StintBurnTarget` to be positive. END validity requires the runtime basis to be valid and the existing `Fuel.RequiredBurnToEnd` to be positive. Invalid runtime basis or selected inputs publish `Target=0.0`, `TargetText=INVALID`, and `TargetValid=false`.
- SESSION validity requires positive numeric `Fuel.LiveLapsRemainingInRace_Stable`, valid non-negative current fuel, positive runtime max tank, non-negative actual MFD/requested add (`Fuel.Pit.WillAdd`, preserving the existing tank-space clamp), non-negative active contingency litres, and positive reserve-adjusted available fuel. SESSION target math is `((min(current fuel + Fuel.Pit.WillAdd, Fuel.MaxTank) - Fuel.Contingency.Litres) / Fuel.LiveLapsRemainingInRace_Stable)`. It intentionally does not use planner fuel values, plugin-recommended `Fuel.Refuel.NextLitresCeil`, or the `_Stable_S` display/smoothing export.
