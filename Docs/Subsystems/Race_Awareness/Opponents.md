...
- Phase split:
  ...
  - **Active pit-cycle phase** (`onPitRoad || pitTripActive`): latches player active-cycle start and uses a simple remaining pit-cycle countdown (`remaining = latched total stop loss - elapsed session time in cycle`, floored at 0) rather than repeatedly treating the stop as a fresh full-loss event. While the cycle is active, the latched total refreshes upward when the shared stop-loss estimate increases. Lower live estimates are accepted only when the pit-service model key changes (selected regulation, NEC factor, refuel amount/rate, tyre count/time, or modeled service overhead); lower estimates from unchanged service inputs, such as repair remaining naturally ageing down, are ignored because elapsed cycle time is already applied in the countdown.
- On active-cycle start, rival pit-road state is baseline-seeded from the current same-class field before transition detection so rivals already on pit road at cycle start are not misclassified as post-start entrants.
