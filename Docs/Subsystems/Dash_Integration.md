...
- Pit Fuel Control ownership/reset contract for dash rendering: only AUTO is plugin-owned; outside AUTO, mode is always MFD truth from `dpFuelFill` (`OFF` unchecked, `MAN` checked). `ModeCycle` remains `OFF -> MAN -> AUTO -> OFF` with explicit raw commands only (no internal `Pit.ToggleFuel`). `OFF -> MAN` explicitly sends `#+fuel$`, then mirrors `MAN STBY`; OFF source/set actions stay isolated (`OFF STBY`, no send). `MAN -> AUTO` sends immediately only for `PUSH/NORM/SAVE`; `STBY` and `PLAN` both enter `AUTO STBY` with no send (PLAN blocked in AUTO switching). `AUTO -> OFF` always attempts `#-fuel$`; failed send leaves AUTO state unchanged, success mirrors OFF truth. PLAN is MAN-only via direct `SetPlan` and additionally requires valid planner/live session match. External MFD OFF/ON/fuel-change actions are monitor+mirror only (no plugin send): AUTO publishes `AUTO REFUEL CANCELLED BY MFD` and mirrors to `OFF STBY`/`MAN STBY`; MAN/OFF publish `REFUEL SET OFF BY MFD`, `REFUEL SET ON BY MFD`, or `FUEL CHANGED BY MFD` and mirror to `OFF STBY`/`MAN STBY`. MAN-owned plugin sends are consumed as owned mirror echoes so delayed telemetry is not misclassified as external takeover. iRacing AutoFuel ownership, Offline Testing suppression, and `Telemetry.IsOnTrackCar` edges still force inert disarmed STBY safety.
