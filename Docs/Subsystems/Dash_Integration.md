...
- Pit command buttons in strategy/pit widgets must bind to plugin-owned actions (`LalaLaunch.Pit.ClearAll`, `LalaLaunch.Pit.ClearTires`, `LalaLaunch.Pit.ToggleFuel`, `LalaLaunch.Pit.FuelSetZero`, `LalaLaunch.Pit.FuelAdd1`, `LalaLaunch.Pit.FuelRemove1`, `LalaLaunch.Pit.FuelAdd10`, `LalaLaunch.Pit.FuelRemove10`, `LalaLaunch.Pit.FuelSetMax`, `LalaLaunch.Pit.ToggleTiresAll`, `LalaLaunch.Pit.ToggleFastRepair`, `LalaLaunch.Pit.ToggleAutoFuel`, `LalaLaunch.Pit.Windshield`, `LalaLaunch.Pit.FuelControl.SourceCycle`, `LalaLaunch.Pit.FuelControl.ModeCycle`, `LalaLaunch.Pit.FuelControl.SetPush`, `LalaLaunch.Pit.FuelControl.SetNorm`, `LalaLaunch.Pit.FuelControl.SetSave`, `LalaLaunch.Pit.TyreControl.ModeCycle`, `LalaLaunch.Pit.TyreControl.SetOff`, `LalaLaunch.Pit.TyreControl.SetDry`, `LalaLaunch.Pit.TyreControl.SetWet`, `LalaLaunch.Pit.TyreControl.SetAuto`) rather than `IRacingExtraProperties` action ids.
...
- Tyre control ON/OFF service truth is four-flag authoritative: ON/OFF is authoritative only when all four tyre flags are available; partial/missing tyre-flag telemetry is unknown/unavailable and service enforcement is held (no retry/attempt spend while unknown).
...
- Pit Fuel Control ownership/reset contract for dash rendering: only AUTO is plugin-owned; outside AUTO, mode must be treated as iRacing-MFD-derived truth from `dpFuelFill` (`OFF` when unchecked, `MAN` when checked). `ModeCycle` contract remains `OFF -> MAN -> AUTO -> OFF` but Fuel Control no longer uses `Pit.ToggleFuel` internally: `OFF -> MAN` is selection-only intent (no send), `MAN -> AUTO` keeps immediate fuel-amount send behavior for `PUSH/NORM/SAVE` and one-shot PLAN behavior, and `AUTO -> OFF` uses explicit raw `#-fuel$` then exits to `Source=STBY` + disarmed on successful send. OFF is a hard guard (no fuel-source send actions are allowed while effective mode is OFF). In AUTO, source cycle excludes PLAN (`PUSH -> NORM -> SAVE -> PUSH`). External-change AUTO cancel remains edge-triggered on `PitSvFuel` and `dpFuelFill` (outside plugin-owned send suppression) and publishes `AUTO CANCELLED` while forcing `Source=STBY` + `AutoArmed=false`. iRacing AutoFuel ownership, Offline Testing suppression, and any `Telemetry.IsOnTrackCar` boolean edge (`false->true` or `true->false`) also force `Source=STBY` + `AutoArmed=false` without plugin-owned OFF/MAN forcing.
