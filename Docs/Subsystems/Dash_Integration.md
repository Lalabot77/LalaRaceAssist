...
- SESSION uses current fuel plus the actual MFD/requested add seam (`Fuel.Pit.WillAdd`, including its existing tank-space clamp), capped by runtime max tank and reduced by active contingency, divided by numeric `Fuel.LiveLapsRemainingInRace_Stable`. It does not assume the plugin-recommended `Fuel.Refuel.NextLitresCeil`; do not use planner fuel values or `Fuel.LiveLapsRemainingInRace_Stable_S` in dashboard formulas for this behavior.
