...
- Pit service regulation math is plugin-owned. Dashboards consume existing pit/strategy exports (`Fuel.Live.TotalStopLoss`, `Pit.Box.*`, `StrategyDash.*`, `PreRace.*`) and the regulation-aware refuel service exports (`Fuel.Live.PitServiceRefuelRate_Lps`, `Fuel.Live.PitServiceRefuelTime_S`) when showing service-rate or “Refuel: xx sec” values; dashboards must not reimplement Default/IMSA/NEC fuel-vs-tyre timing formulas. `Fuel.Live.RefuelRate_Lps` remains the normal physical/profile rate for compatibility and must not be interpreted as regulation-adjusted.
