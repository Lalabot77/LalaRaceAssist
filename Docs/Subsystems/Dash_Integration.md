...
- PreRace status is scenario-first (`required strategy` vs `selected strategy`) and fully partitioned:
  ...
  - required `One Stop` => no-stop `SINGLE STINT NOT POSSIBLE`, one-stop feasibility (pit-stop refill-capacity gate, then `SINGLE STOP POSSIBLE` when next-stint refuel setup is needed, `OVERFUELLED`, or `SINGLE STOP OKAY`), multi-stop `SINGLE STOP POSSIBLE`,
