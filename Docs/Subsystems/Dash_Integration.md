...
- `LalaLaunch.PreRace.*` and `StrategyDash.*` pre-green guidance includes Formation Lap Fuel (grid/start requirement). `StrategyDash.StartFuelRequiredLitres` means fuel required to join grid/start with and is capped by effective tank capacity when published.
