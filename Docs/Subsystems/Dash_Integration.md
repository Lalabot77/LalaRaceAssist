...
- Tyre control display contract: outside AUTO (`OFF`/`DRY`/`WET`) those exports are bounded truth-synced with MFD state (`Pit.ToggleTyresAll`-aligned service truth + `PitSvTireCompound` dry/wet family mapping) **before** manual enforcement (so stale manual intent does not fight external MFD truth); `ResetToOff()` safety resets remain latched at `OFF` and do not immediately remap back to `DRY/WET` on the next tick. AUTO remains plugin-owned authority and can show info-only unconfirmed feedback without mode collapse.
