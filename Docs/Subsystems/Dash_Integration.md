...
- `RaceFinish.PlayerOverallFieldSize` / `RaceFinish.PlayerClassFieldSize` freeze the corresponding live `Race.*` counts at class snapshot (not player snapshot) so `Pxx / yy` denominators do not drift when drivers quit before player finish.
