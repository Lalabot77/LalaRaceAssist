...
- Pit Fuel Control ownership/reset contract for dash rendering: only AUTO is plugin-owned; outside AUTO, mode is always MFD truth from `dpFuelFill` (`OFF` unchecked, `MAN` checked). `ModeCycle` remains `OFF -> MAN -> AUTO -> OFF` with explicit raw commands only (no internal `Pit.ToggleFuel`). `OFF -> MAN` is `FUEL MAN STBY` with no send. OFF is a hard guard and all source/set actions are isolated (`OFF STBY`, no send). `MAN -> AUTO` sends immediately only for `PUSH/NORM/SAVE`; `STBY` and `PLAN` both enter `AUTO STBY` with no send (PLAN blocked in AUTO switching). `AUTO -> OFF` always attempts `#-fuel$`; failed send falls back to `AUTO STBY`, success mirrors OFF truth. PLAN is MAN-only via direct `SetPlan`. External MFD OFF/ON/fuel-change actions are monitor+mirror only (no plugin send): AUTO publishes `AUTO REFUEL CANCELLED BY MFD` and mirrors to `OFF STBY`/`MAN STBY`; MAN/OFF publish `REFUEL SET OFF BY MFD`, `REFUEL SET ON BY MFD`, or `FUEL CHANGED BY MFD` and mirror to `OFF STBY`/`MAN STBY`. iRacing AutoFuel ownership, Offline Testing suppression, and `Telemetry.IsOnTrackCar` edges still force inert disarmed STBY safety.
