...
- Smooth pit-window range bars should use `Fuel.PitWindow.SelectedOpeningLap` / `Fuel.PitWindow.SelectedClosingLap` when they need decimal lap endpoints that follow the selected runtime PUSH/NORM/SAVE burn basis. Keep `Fuel.PitWindowOpeningLap` / `Fuel.PitWindowClosingLap` for the existing state-machine integer markers and do not rebuild plugin fuel-window math in dashboard formulas.
