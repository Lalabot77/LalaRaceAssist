...
- Tyre control display contract: outside AUTO (`OFF`/`DRY`/`WET`) those exports are bounded truth-synced with MFD state (`Pit.ToggleTyresAll`-aligned service truth + `PitSvTireCompound` dry/wet family mapping), while AUTO remains plugin-owned authority and can show info-only unconfirmed feedback without mode collapse.
