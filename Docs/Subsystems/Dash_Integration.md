...
- `LalaLaunch.PreRace.FuelSource` / `LapTimeSource` / `LiveFacingBasisText` contract:
  - `LiveFacingBasisText` format is `DATA <LIVE|PLAN> | LAP <LIVE|PROFILE|SIM|PLAN|DEFAULT> / BURN <LIVE|PROFILE|SIM|DEFAULT>`,
...
- `RaceFinish.PlayerOverallFieldSize` / `RaceFinish.PlayerClassFieldSize` freeze the corresponding live `Race.*` counts at class snapshot (not player snapshot) so `Pxx / yy` denominators do not drift when drivers quit before player finish. `Race.PlayerClassFieldSize` denominator is current-session cohort/native telemetry based and does not use CSV registered class-membership counts.
