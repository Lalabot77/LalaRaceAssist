...
- **Slot 01 precision gaps:** `Car.Ahead01P.Gap.Sec` / `Car.Behind01P.Gap.Sec` remain sharper textual/number-display values for the closest ahead/behind slot: fresh gate truth first, then defensible filtered proximity when truth is stale, then track-gap fallback. They bypass `Gap.RelativeSec` sticky-hold semantics and never publish stale raw truth indefinitely.
