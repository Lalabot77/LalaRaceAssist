...
- Both exports intentionally suppress evaluation during each subsystem’s existing post-command settle/suppression windows to avoid normal latency flash. Pit Fuel Control also exposes MsgCx as a no-command recovery check that clears only non-zero fuel-control fault state or pending requests that are already stale by the confirmation-expiry rules; it is not a normal pending-command cancel.
