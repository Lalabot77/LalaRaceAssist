...
- Both exports intentionally suppress evaluation during each subsystem’s existing post-command settle/suppression windows to avoid normal latency flash. Pit Fuel Control also exposes MsgCx as a no-command recovery check that clears only pending requests that are already stale/expired by the confirmation-expiry rules; it is not a live fault clear and not a normal pending-command cancel.
